...
## SURFACE 8 · /build-your-plan — Build Your Plan configurator (HEAD target 7010e71)
**Purpose:** customer-facing self-serve plan configurator. Operator picks Base (Prudent360 ERP / Module / Bundle) → Add-ons → Conditions (companies/users/space) → Plan & Price (model · billing cycle · margin · discount) with a **live quote**, then submits a `ProvisionRequest` that drops into the provisioning queue. **Known-deferred** per prompt: real checkout / payment / instant provisioning = Wave-2 (record, don't fail).

| Check | Result | Evidence | Data type |
|---|---|---|---|
| Page loads (no auth gate — public configurator) | **PASS** | `/build-your-plan` renders sticky header ("Build Your Plan · SP.4 · Tier-L badge · Live quote ₹X/mo"), honest warning banner `"Live quote · request drops into the provisioning queue · checkout, payment & instant provisioning arrive with Wave-2."`, 5-step stepper (Base · Add-ons · Conditions · Plan & Price · Request), main column + 320px sticky summary sidebar. ENTITY scope = `'public-build-your-plan'`. | — |
| Stepper navigation | **PASS** | Pills are buttons that jump directly to any step; Back/Next footer also moves the cursor and respects `canNext` (Base requires ≥1 card / non-empty module/bundle). | — |
| Base step — ERP preset | **PASS** | Default `baseKind='erp'` with `CORE_ERP_PRESET = ['fincore','salesx','procure360','inventory-hub','insightx','command-center']` pre-selected (6 cards). Card-toggle grid is built from `ALL_CARD_IDS` (real CardId union — no fabricated). Switching to Module/Bundle reads from `VARIANT_MODULE_IDS` / `BUNDLE_IDS` (`id.startsWith('bundle-')` split). | **LIVE** — derived from `product-variant-engine` catalogue exports. |
| Add-ons step | **PASS** | Toggle grid built from `VARIANT_ADDON_IDS` (real 12-addon catalogue). Toggling pushes/pops `enabledAddons`; summary line in plan-summary re-renders. | **LIVE** — catalogue-driven. |
| Conditions step — limits drive price | **PASS** | Sliders/inputs bind to `LimitSet{companies, users, space_gb}` (start 1/10/25). Changing `users` re-fires `useMemo([effectivePricing, limits])` → live price updates. | **LIVE**. |
| **Live quote math — independent drill** | **PASS** | Pure-JS replay of `computeListPrice` with defaults (`hybrid · base 4999 · per_user 499 · per_company 1999 · per_gb 25 · companies 1 / users 10 / space 25 GB`): **monthly = ₹12,613** (= 4999 + 499·10 + 1999·1 + 25·25). Billing-cycle overlays applied via `cycleDiscount` (annual=10%, multi_year=18%): **annual = ₹11,352** (`round(12613·0.9)`), **multi_year = ₹10,343** (`round(12613·0.82)`). Bump users 10→25 → **₹20,098**. `computeChannelPrice(12613, 15)` → **₹10,721**. All match `Math.round(subtotal·(1 − discount/100))` exactly. | **LIVE** — every input change re-derives the quote via `useMemo`. |
| Header `Live quote ₹X/{mo|yr|3yr}` ticker | **PASS** | Source line 171 binds to `listPrice.toLocaleString('en-IN')` + cycle suffix; updates immediately on cycle change because `effectivePricing` is a memo over `[pricing, cycleDiscount]` and `listPrice` is a memo over `[effectivePricing, limits]`. | **LIVE**. |
| Plan & Price step — model/billing/margin controls | **PASS** | Selects expose all 5 `PricingModel` (per_seat · per_company · flat_tier · usage · hybrid), all 3 `BillingCycle` (monthly · annual · multi_year), discount % and channel_margin % numeric inputs. `flat_tier` correctly ignores per-unit multipliers (engine `case 'flat_tier': break`). `usage` adds only `per_gb_price · space_gb`. Verified by drill. | **LIVE** math. |
| **Request step — CTA drops into provisioning queue** | **PASS** | `RequestStep` exposes `ProvisionRequestType` select (demo / final_copy / channel_partner / client / client_of_partner), `requester_name` (required — Submit disabled when blank), `notes` textarea. Click invokes `handleSubmit()` → `createProvisionRequest('public-build-your-plan', {type, requester_name, notes: planSummary() + notes})` → writes to `localStorage['tower_provision_requests_public-build-your-plan']` (key from `provisionRequestsKey`). Returned `ProvisionRequest` has `id` (rid('pr')), `status: 'requested'`, `created_at: nowIso()`. UI swaps to `ConfirmationView` with the request id + plan summary + reset CTA. | **LIVE** — actually persists. |
| **Request persistence (vitest drill)** | **PASS** | `bunx vitest run src/test/sprint-sp4/sp4-block-behavioral.test.ts` → **29/29 green** (transform 556ms · tests 21ms). The suite exercises `createProvisionRequest` round-trip, `listProvisionRequests` read-back, and the forward-only status guard (`requested → approved → provisioned → active`, plus `active ↔ suspended` recovery; skipping steps rejected). Confirms the CTA wires to a real persistent queue, not fire-and-forget. | **LIVE**. |
| **Queue visibility in Tower** | **PARTIAL (honest scope finding)** | `src/pages/tower/ProvisioningManager.tsx` reads from `provisionRequestsKey(entity)` but **scopes to its own entity context** (active tower entity), whereas `BuildYourPlanPage` always writes under the fixed `'public-build-your-plan'` scope. Submitted requests land in localStorage correctly and are readable by `listProvisionRequests('public-build-your-plan')`, but **they will not appear in the Tower's Provisioning Manager unless it is pointed at the `public-build-your-plan` scope** (or a cross-scope inbox is added). Recorded as a wiring follow-up, not a failure of the configurator itself. | **LIVE** but scope-mismatched. |
| Real checkout / payment / instant provisioning | **CNR — Wave-2 (deferred per prompt)** | Confirmed absent by design. Honest amber banner at the top states it verbatim: *"Live quote · request drops into the provisioning queue · checkout, payment & instant provisioning arrive with Wave-2."* No Stripe/Razorpay/Paddle wiring, no instant-tenant spin-up, no entitlement enforcement. Source comment line 11: *"NO checkout · NO payment · NO instant provisioning (Wave-2)."* Per prompt: **record, don't fail**. | — |
| Honest banner present | **PASS** | `BUILD_YOUR_PLAN_HONESTY` constant rendered in a warning-tinted card with `AlertTriangle` icon at the top of every step. | **STATIC** copy. |
| Plan summary string (`planSummary()`) | **PASS** | Composes `Base: … | Add-ons: … | Companies/Users/Space | Pricing model: hybrid · monthly · list ₹12613`. Embedded into the request `notes` field so reviewers see the full snapshot. | **LIVE** — recomputed at submit. |
| Theme | **PASS** | Uses `bg-background`, `bg-card`, `text-foreground`, `text-muted-foreground`, `border-border`, `bg-primary/text-primary-foreground` throughout — no hardcoded hex/dark-only colors. Follows global toggle. Honest banner uses `warning` semantic token. | — |

**Console:** only the pre-existing React Router v6→v7 future-flag warnings; no errors specific to the configurator.

**Surface 8 verdict:** **PASS** — Build Your Plan is a **functionally live** configurator, not a mock. The live quote is real math (`computeListPrice` / `computeChannelPrice`) that reacts to every catalogue/limit/pricing/billing-cycle change (verified by independent arithmetic drill: ₹12,613 → ₹11,352 / ₹10,343 / ₹20,098 / channel ₹10,721 for the documented inputs). The CTA actually persists a `ProvisionRequest` (status `requested`) into `localStorage['tower_provision_requests_public-build-your-plan']` via `createProvisionRequest`, and 29/29 SP.4 behavioral tests confirm the round-trip + the forward-only status guard. Wave-2 deferrals (checkout / payment / instant provisioning) are surfaced honestly in an amber banner and in source comments — recorded, not failed, per prompt. ⚠️ **Honest scope finding:** submitted requests land under the fixed `'public-build-your-plan'` entity scope, whereas the Tower Provisioning Manager reads from its active entity scope — requests are persisted but not yet visible in Tower until a cross-scope inbox (or a fixed `'public-build-your-plan'` reader) is added. Recommend a follow-up sprint to surface the public configurator queue inside Tower.

## PROGRESS LEDGER (final)
```
DONE: [Login, /tower/dashboard, /bridge/dashboard, /partner/dashboard, /customer/dashboard, /welcome/scenarios, /welcome/dev-tools, /build-your-plan]   NEXT: —   REMAINING: 0   STATUS: SMOKE RUN COMPLETE
ORDER: 1.Login✓ 2./tower/dashboard✓ 3./bridge/dashboard✓ 4./partner/dashboard✓ 5./customer/dashboard✓ 6./welcome/scenarios✓ 7./welcome/dev-tools✓ 8./build-your-plan✓
```

**STOP — final surface complete. Smoke run closed.**
