/**
 * SalesOrderRegister.tsx — UPRA-4 Phase B · T1-1 NEW canonical Register
 * Canonical UniversalRegisterGrid<Order> consumer · filtered to base_voucher_type === 'Sales Order'.
 * Sidebar route: sx-r-so-register (NEW additive · SalesXPage switch case)
 ...
 * Export name: SalesOrderRegisterPanel (named export · no default per UPRA-4 Phase A precedent).
 * [JWT] GET /api/orders?entity={code}&type=SalesOrder
 */
...
export function SalesOrderRegisterPanel(): JSX.Element {
  ...
  const meta: RegisterMeta<Order> = {
    ...
    description: 'All Sales Orders · commitment documents · zero GL/stock/GST impact · Tally-Prime register',
    dateAccessor: r => r.date,
  };

  const columns: RegisterColumn<Order>[] = [
    { key: 'order_no', label: 'SO No', clickable: true, render: r => r.order_no, exportKey: 'order_no' },
    { key: 'date', label: 'Date', render: r => r.date, exportKey: 'date' },
    { key: 'party', label: 'Customer', render: r => r.party_name, exportKey: 'party_name' },
    { key: 'gstin', label: 'GSTIN', render: r => r.party_gstin ?? '—', exportKey: r => r.party_gstin ?? '' },
    { key: 'ref', label: 'Customer PO', render: r => r.ref_no ?? '—', exportKey: r => r.ref_no ?? '' },
    ...
    <div className="max-w-7xl mx-auto space-y-4 p-6">
      ...
      <Dialog open={!!selected} onOpenChange={o => !o && setSelected(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          {selected && <SalesOrderDetailPanel order={selected} onPrint={() => setPrinting(selected)} />}
        </DialogContent>
      </Dialog>
