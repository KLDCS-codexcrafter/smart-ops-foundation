...
export function QuotationRegisterV2Panel({ initialFilter }: QuotationRegisterV2PanelProps = {}) {
  ...
  const chartRows = useMemo(() => {
    ...
    for (const q of allQs) m.set(q.quotation_stage, (m.get(q.quotation_stage) ?? 0) + 1);
    return Array.from(m.entries()).map(([status, count]) => ({ status, count }));
  }, [allQs]);
  const chartConfig = getKpi('sx-quotations')?.defaultChart ?? defaultChartConfig({
    chartType: 'doughnut', xKey: 'status',
    series: [{ key: 'count', label: 'Quotations' }],
    title: 'Quotations by stage',
  });
