...
export function SRMRegisterPanel({ initialFilter }: SRMRegisterPanelProps = {}) {
  ...
  const chartRows = useMemo(() => {
    ...
    for (const s of allSrms) m.set(s.status, (m.get(s.status) ?? 0) + 1);
    return Array.from(m.entries()).map(([status, count]) => ({ status, count }));
  }, [allSrms]);
  const chartConfig = getKpi('sx-srm')?.defaultChart ?? defaultChartConfig({
    chartType: 'column', xKey: 'status',
    series: [{ key: 'count', label: 'SRMs' }],
    title: 'SRMs by status',
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
      ...
      <Card className="p-3 space-y-2" data-testid="sx-srm-dashboard-host">
        <div className="flex items-center gap-2 flex-wrap">
          <Badge variant="outline" className="text-[10px] font-mono" data-testid="sx-srm-integrity-badge" title={integrityHash}>
            <ShieldCheck className="h-3 w-3 mr-1" />{shortHash}
          </Badge>
        </div>
        {chartRows.length === 0 ? (
          <div className="text-sm text-muted-foreground py-6 text-center">No SRMs yet</div>
        ) : (
          <div className="w-full h-72" data-testid="sx-srm-chart-host">
