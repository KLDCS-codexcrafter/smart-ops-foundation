/**
 * @file     CreditNoteRegister.tsx
 * @purpose  Credit Note Register — sales returns with GST reversal columns.
 ...
 * @consumers FinCorePage (switch 'fc-rpt-credit-note-register')
 */
...
interface CreditNoteRegisterProps {
  entityCode: string;
...
export function CreditNoteRegisterPanel({ entityCode, onNavigate }: CreditNoteRegisterProps) {
  ...
  const meta: RegisterMeta = useMemo(() => ({
    registerCode: 'credit_note_register',
    title: 'Credit Note Register',
    voucherFilter: v => v.base_voucher_type === 'Credit Note',
    drillDownType: 'Credit Note',
  }), []);
...
export default CreditNoteRegisterPanel;
