/**
 * @file     ReceiptRegister.tsx
 * @purpose  Receipt Register — chronological listing of all Receipt vouchers.
 ...
 * @consumers FinCorePage (switch 'fc-rpt-receipt-register')
 */
...
export function ReceiptRegisterPanel({ entityCode, onNavigate }: ReceiptRegisterProps) {
  const columns: RegisterColumn<Voucher>[] = useMemo(() => [
    ...
    { key: 'bank',   label: 'Bank',          render: v => v.bank_name ?? v.to_ledger_name ?? '—', exportKey: v => v.bank_name ?? v.to_ledger_name ?? '' },
  ...
  const meta: RegisterMeta = useMemo(() => ({
    ...
    voucherFilter: v => v.base_voucher_type === 'Receipt',
    drillDownType: 'Receipt',
  }), []);
  ...
  const summaryBuilder = (filtered: Voucher[]): SummaryCard[] => {
    ...
    return [
      ...
      { label: 'Total Amount', value: inr(total), tone: 'positive' },
...
export default ReceiptRegisterPanel;
