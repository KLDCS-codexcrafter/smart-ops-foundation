/**
 * @file        PurchaseCostVarianceItemPanel.tsx
 * @purpose     Tier-1 purchase cost variance · per-item dimension.
 * @sprint      T-Phase-1.A.3.d-Procure360-Variance-Trident-Polish
 * @decisions   D-NEW-AR · D-NEW-AS
 * @reuses      purchase-cost-variance-engine · useEntityCode · decimal-helpers · UI primitives
 * @[JWT]       GET /api/procure360/reports/cost-variance/item — localStorage-backed in Phase 1
 */
...
export function PurchaseCostVarianceItemPanel(): JSX.Element {
  const { entityCode } = useEntityCode();
  const variances = useMemo<PurchaseCostVariance[]>(
    () => listAllPurchaseCostVariances(entityCode, 'item'),
    [entityCode],
  );
  ...
  return (
    <div className="p-6 space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Purchase Cost Variance · Item</h1>
        <p className="text-sm text-muted-foreground">
          Tier 1 of 3 · per-item rolling 90d window · reference: contract → std purchase → rolling avg.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Items With Variance</CardTitle></CardHeader>
      ...
      <Card>
        <CardContent className="p-0">
          {variances.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted-foreground">No item variances yet · need PO history.</div>
          ) : (
