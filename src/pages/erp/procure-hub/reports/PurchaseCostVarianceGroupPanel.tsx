/**
 * @file        PurchaseCostVarianceGroupPanel.tsx
 * @purpose     Tier-2 purchase cost variance · per stock-group dimension.
 ...
 * @[JWT]       GET /api/procure360/reports/cost-variance/group — localStorage-backed in Phase 1
 */
...
export function PurchaseCostVarianceGroupPanel(): JSX.Element {
  const { entityCode } = useEntityCode();
  const variances = useMemo<PurchaseCostVariance[]>(
    () => listAllPurchaseCostVariances(entityCode, 'group'),
    [entityCode],
  );
  ...
  return (
    <div className="p-6 space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Purchase Cost Variance · Group</h1>
        <p className="text-sm text-muted-foreground">
          Tier 2 of 3 · per stock-group dimension · weighted aggregate of item-level variance.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Groups With Variance</CardTitle></CardHeader>
      ...
      <Card>
        <CardContent className="p-0">
          {variances.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted-foreground">No group variances yet · need PO history.</div>
          ) : (
