...
export function PurchaseCostVarianceCategoryPanel(): JSX.Element {
  ...
  const chartRows = useMemo(
    () => [...variances]
      ...
      .slice(0, 12)
      .map((v) => ({ category: v.dimension_name, variance: v.variance_amount })),
    [variances],
  );
  const chartConfig = getKpi('pr-cost-variance-cat')?.defaultChart ?? defaultChartConfig({
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <Card className="p-3 space-y-2" data-testid="pr-cost-variance-cat-dashboard-host">
        <div className="flex items-center gap-2 flex-wrap">
          <Badge variant="outline" className="text-[10px] font-mono" data-testid="pr-cost-variance-cat-integrity-badge" title={integrityHash}>
            <ShieldCheck className="h-3 w-3 mr-1" />{shortHash}
          </Badge>
        </div>
        {chartRows.length === 0 ? (
          <div className="text-sm text-muted-foreground py-6 text-center">No category variances yet</div>
        ) : (
          <div className="w-full h-72" data-testid="pr-cost-variance-cat-chart-host">
