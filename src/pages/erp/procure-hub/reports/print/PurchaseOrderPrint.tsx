/**
 * PurchaseOrderPrint.tsx — UPRA-4 Phase B · A4 portrait Purchase Order document.
 */
...
export function PurchaseOrderPrint({ order, onClose }: PurchaseOrderPrintProps): JSX.Element {
  ...
  return (
    <UniversalPrintFrame
      ...
      onClose={onClose}
      signatories={['Procurement Officer', 'Procurement Manager', 'Authorised Signatory']}
      referenceBlock={
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div><span className="font-semibold">Vendor:</span> {order.party_name}</div>
          <div><span className="font-semibold">GSTIN:</span> {order.party_gstin ?? 'Unregistered'}</div>
          <div><span className="font-semibold">Indent/RFQ Ref:</span> {order.ref_no ?? '—'} {order.ref_date && `(${order.ref_date})`}</div>
      ...
      termsAndConditions={order.terms_conditions || 'Purchase Order · commitment document only · zero GL/stock/GST impact at this stage (downstream GRN/Bill Passing will create the postings).'}
    >
      <Table>
        <TableHeader>
          <TableRow>
