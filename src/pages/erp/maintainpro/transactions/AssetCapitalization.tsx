...
export function AssetCapitalization(_props: Props): JSX.Element {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <p className="text-sm text-muted-foreground">SiteX CAPEX handoff → Equipment + Asset Cap voucher (FinCore stub)</p>
        ...
        <div className="space-y-2 text-xs font-mono">
          {list.map((c) => (
            <div key={c.id} className="flex items-center justify-between py-2 border-b border-border">
              ...
              {!c.fincore_voucher_id && <Badge variant="outline" className="text-muted-foreground">FinCore stub</Badge>}
            </div>
          ))}
