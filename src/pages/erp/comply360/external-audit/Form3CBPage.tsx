/**
 * @file        src/pages/erp/comply360/external-audit/Form3CBPage.tsx
 * @purpose     Form 3CB surface · used when no statutory audit applies (firms, proprietorships)
...
import { build3CB, type EntityMeta, type AuditorMeta } from '@/lib/comply360-tax-audit-3cd-engine';
import { useEntityCode } from '@/hooks/useEntityCode';

export default function Form3CBPage(): JSX.Element {
  ...
  const handleBuild = (): void => {
    ...
    toast.success(`Form 3CB built · ${form.qualifications.length} qualification(s)`);
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div>
        <h1 className="text-2xl font-semibold">Form 3CB · Tax Audit Report</h1>
        <p className="text-muted-foreground text-sm">For entities without a separate statutory audit (firms, proprietorships).</p>
      </div>
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="space-y-1"><Label>PAN</Label>
            <Input value={entity.pan} onChange={(e) => setEntity({ ...entity, pan: e.target.value.toUpperCase() })} /></div>
      ...
      <Card className="p-4">
        ...
        <div className="mt-3">
          <Button onClick={handleBuild}><FileText className="h-4 w-4 mr-1" /> Build Form 3CB</Button>
        </div>
      </Card>
    </div>
  );
