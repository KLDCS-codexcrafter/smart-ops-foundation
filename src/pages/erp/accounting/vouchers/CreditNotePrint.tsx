...
export function CreditNotePrintPanel() {
  ...
  const copyKey = params.get('copy') ?? CREDIT_NOTE_COPY_CONFIG.default;

  const [payload, setPayload] = useState<CreditNotePrintPayload | null>(null);
  ...
  const content = useMemo(() => {
    ...
    <>
      <div className="no-print px-4 py-2 border-b border-border bg-muted/20">
        <DocSendBar objectType="invoice-memo" sourceCard="fincore" sourceRecord={(voucher as unknown) as Record<string, unknown>} />
      </div>
      ...
      <>
        <div className="grid grid-cols-2 gap-4 text-[11px]">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1">
            <div className="text-muted-foreground">CN No</div>
            <div className="font-mono text-right">{payload.voucher_no}</div>
            <div className="text-muted-foreground">CN Date</div>
...
export default CreditNotePrintPanel;
