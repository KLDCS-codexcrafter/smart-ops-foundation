/**
 * ManufacturingJournalPrint.tsx — A4 printable Manufacturing Journal voucher.
 */
...
function Section({ title, lines, total, prefix }: {
  title: string; lines: MfgJournalPrintLine[]; total: number; prefix: string;
}) {
  return (
    <div className="mt-4">
        ...
        <table className="w-full text-[10px] border-collapse">
          ...
          <tbody>
            ...
            <tr className="bg-muted/20 font-semibold">
              <td className="border border-border p-1.5" colSpan={4}>Subtotal</td>
...
export function ManufacturingJournalPrintPanel() {
  ...
  useEffect(() => {
    ...
    setPayload(buildMfgJournalPrintPayload(voucher, gst, copyKey));
  ...
  const content = useMemo(() => {
    ...
    return (
      <>
        ...
        <Section title="Consumption" lines={payload.consumption_lines} total={payload.total_consumption_qty} prefix="mj-c" />
        <Section title="Production" lines={payload.production_lines} total={payload.total_production_qty} prefix="mj-p" />
        <Section title="Byproducts" lines={payload.byproduct_lines} total={payload.total_byproduct_qty} prefix="mj-b" />
