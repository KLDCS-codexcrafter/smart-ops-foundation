/**
 * @file     ReceiptPrint.tsx
 * @purpose  A4 printable Receipt voucher — 1-copy (Accounts).
 ...
 * @depends  receipt-print-engine.ts · print-config-storage.ts · PrintSheetFrame
 */
...
import {
  buildReceiptPrintPayload, RECEIPT_COPY_CONFIG,
  formatINR, formatDDMMMYYYY,
  buildReceiptExportRows,
  type ReceiptPrintPayload,
} from '@/lib/receipt-print-engine';
...
export function ReceiptPrintPanel() {
  ...
  const copyKey = params.get('copy') ?? RECEIPT_COPY_CONFIG.default;
  ...
  useEffect(() => {
    ...
    setPayload(buildReceiptPrintPayload(voucher, gst, copyKey, printConfig));
    ...
    return (
      <>
        ...
        <div className="mt-3 border-t border-border pt-3 text-[11px]">
          <div className="text-[9px] uppercase tracking-wider text-muted-foreground">Received From</div>
        ...
        <div className="mt-4 border-t border-border pt-2 flex items-end justify-between text-[11px]">
          ...
          <div className="text-right">
            <div className="text-[9px] uppercase tracking-wider text-muted-foreground">Total Received</div>
  ...
  return (
    <PrintSheetFrame
      ...
      exportData={payload ? { payload, buildRows: (p) => buildReceiptExportRows(p as ReceiptPrintPayload) } : undefined}
    >
      {content}
    </PrintSheetFrame>
  );
