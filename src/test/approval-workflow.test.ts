...
import {
  ...
  type ApprovalContext, type ApprovalRecord,
...
const mk = (status: string): ApprovalRecord => ({ id: 'r1', status });
...
describe('approval-workflow-engine · M-4', () => {
  ...
  it('AW9 · cancelApproval moves any → cancelled with reason', () => {
    const r = cancelApproval(mk('posted'), 'reversal', ctx);
  ...
  it('AW10 · cancelApproval rejects already-cancelled', () => {
    const r = cancelApproval(mk('cancelled'), 'x', ctx);
    expect(r.ok).toBe(false);
  });
  it('AW11 · custom field map redirects to approved_by_id', () => {
    ...
    const r = approve(mk('submitted'), { id: 'a1', name: 'Mgr' }, customCtx);
  ...
  it('AW12 · audit before/after states differ correctly', () => {
    submit(mk('draft'), { id: 'u1', name: 'A' }, ctx);
