/**
 * @file     debit-note-print-engine.ts
 * @purpose  Build print payload for purchase-side Debit Note · 3-copy (Original / Duplicate / Triplicate).
 ...
 * @whom     Vendor (recipient) · Transporter · Supplier (us)
 * @depends  voucher-print-shared.ts · Voucher · EntityGSTConfig · print-config.ts · print-config-storage.ts
 * @consumers DebitNotePrint.tsx panel (2b.3b-B2 wires toggle-gating)
 */
...
export const DEBIT_NOTE_COPY_CONFIG: PrintCopyConfig = {
...
export interface DebitNotePrintLine {
  sl_no: number;
  item_code: string;
...
export interface DebitNotePrintHsnSummary {
  hsn_sac: string;
  taxable: number;
  cgst: number;
  sgst: number;
...
export interface DebitNotePrintPayload {
  copy_key: string;
  copy_label: string;
  ...
  voucher_no: string;
  voucher_date: string;
  against_bill_no: string;
  against_bill_date: string | null;
  reason_code: string;
  place_of_supply: string;

  lines: DebitNotePrintLine[];
  hsn_summary: DebitNotePrintHsnSummary[];
...
export function buildDebitNotePrintPayload(
  voucher: Voucher,
  issuerGst: EntityGSTConfig,
  copyKey: string = DEBIT_NOTE_COPY_CONFIG.default,
  config?: PrintConfig,
): DebitNotePrintPayload {
  const issuer = buildSupplierBlock(issuerGst);

  const lines: DebitNotePrintLine[] = (voucher.inventory_lines ?? []).map((l, i) => ({
    sl_no: i + 1,
  ...
  const hsnMap = new Map<string, DebitNotePrintHsnSummary>();
  ...
  const resolved_toggles = resolveToggles(config ?? DEFAULT_PRINT_CONFIG, 'debit_note');

  return {
    copy_key: copyKey,
    copy_label: DEBIT_NOTE_COPY_CONFIG.labels[copyKey] ?? DEBIT_NOTE_COPY_CONFIG.labels[DEBIT_NOTE_COPY_CONFIG.default],
    ...
    recipient_state: voucher.party_state_code ?? '',

    voucher_no: voucher.voucher_no,
    voucher_date: voucher.date,
    against_bill_no: voucher.ref_voucher_no || '',
    against_bill_date: null,
    reason_code: '—',
...
/**
 * @purpose   Transform Debit Note payload into tabular ExportRows for CSV/XLSX export.
 *            Multi-sheet: Debit Note (lines) + HSN Summary.
 ...
 */
export function buildDebitNoteExportRows(payload: DebitNotePrintPayload): ExportRows {
  const metaRows: (string | number | null)[][] = [
    ['Debit Note No',   payload.voucher_no],
    ...
    ['Against Bill',    payload.against_bill_no],
