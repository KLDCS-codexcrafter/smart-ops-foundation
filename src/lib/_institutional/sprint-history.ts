...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🆕 Sprint SP.3 T-SP3-Provisioning · SaaS Productization · Request-Intake & Provisioning Manager + 4-level Account Hierarchy · NEW SIBLING provisioning-engine · 4-level hierarchy (super-admin → {direct client | channel-partner → client-of-partner}) with honest validation (single super_admin root · client_of_partner requires partner_id pointing to a channel_partner · no cycles · parent must exist) · 5 ProvisionRequest types (demo · final_copy · channel_partner · client · client_of_partner) · 5-status guarded lifecycle (requested→approved→provisioned→active↔suspended · no skip transitions enforced by canTransition · forward-only) · importPartnerClientsAsNodes CONSUMES partner-portal-engine getPartnerCustomers read-only (no duplicate partner store · idempotent skip-by-id) · approveAndProvision CONSUMES product-variant-engine assignVariantToTenant (which in turn seeds CardEntitlement via card-entitlement-engine seedDemoEntitlements — full delegation chain · NO in-engine CardEntitlement fabrication) · creates matching AccountNode (skipped for demo sandbox) under super-admin root · status flips to provisioned + provisioned_at stamped · convertDemoToFinal flips type demo→final_copy preserving variant · lightweight in-engine audit log capped at 500 rows · Manager page mounts in /tower as new "Provisioning" sidebar entry (additive · TowerLayout 0-DIFF except nav array) · request queue with type/status filter + variant dropdown (only published variants) + approve/provision/activate/suspend/convert actions · hierarchy tree view with "Import partner clients" action on channel_partner nodes · honest Wave-2 banner mounted at top of page (PROVISIONING_HONESTY) · provisioning = STATUS-TRACK ONLY (Tier-L) · NO real instance spin-up · NO entitlement enforcement · NO billing/charging · NO per-account auth (all Wave-2 · grep confirms no spinUpInstance/enforceLimit/chargeBilling) · walls held: product-variant-engine + partner-portal-engine + card-entitlement-engine + card-entitlement.ts + Tower Tenants.tsx + applications.ts + hash-chain + retention all 0-DIFF · SP.2 flipped to 9a17efe6 · 112→113 ⭐ · headSha TBD_AT_BANK
  {
    ...
    headSha: 'f02c930c', predecessorSha: '9a17efe6', loc: 950,
    newSiblings: ['provisioning-engine'],
    bankDate: '2026-06-09', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint SP.4 T-SP4-Build-Your-Plan · SaaS Productization ARC CLOSE · SP.1 Variant Builder + SP.2 Prudent360 ERP/LimitSet/Pricing + SP.3 Provisioning/Hierarchy + SP.4 self-serve Build-Your-Plan configurator · live price (display) → ProvisionRequest · CONSUMES pricing + provisioning · checkout/payment/real-provisioning = Wave-2 · NEW public route /build-your-plan + /welcome tile · NO new SIBLING (consuming UI · empty newSiblings) · CONSUMES product-variant-engine computeListPrice/computeChannelPrice (SP.2 · display math · greppable delegation) + provisioning-engine createProvisionRequest (SP.3 · queue intake · greppable delegation) + ALL_CARD_IDS + VARIANT_MODULE_IDS + VARIANT_ADDON_IDS (catalog 0-DIFF) · stepwise composer (Base ERP-cards|module|bundle → Add-ons → Conditions companies/users/space → Plan & Price billing-cycle + per-model math → Request CTA demo/final_copy/client) · sticky live-quote sidebar · honest Wave-2 banner mounted top + confirmation footer · public/Tier-L (no <P> wrap · no auth · no checkout · no payment · no instant provisioning) · walls held: product-variant-engine + provisioning-engine + card-entitlement.ts + ModulesPage/AddOnsPage catalog + applications.ts + hash-chain + retention all 0-DIFF · SP.3 flipped to f02c930c · 113→114 ⭐ · headSha 541b4b63
  {
    sprintNumber: 'SP4' as unknown as number, code: 'T-SP4-Build-Your-Plan', composite: false, grade: 'A',
    headSha: '541b4b63', predecessorSha: 'f02c930c', loc: 900,
    newSiblings: [],
    bankDate: '2026-06-09', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint AM.1 T-AM1-AI-Everywhere · best-of-both Tier-L · Dishani department-aware via route→card context-resolver (NEW dishani-context-resolver.ts + useDishaniRouteContext hook · NO per-card-page edits · 33 ERP cards + 5 panels covered via ROUTE_CARD_REGISTRY · longest-prefix match · deterministic intent-match answerCardScoped with 6 classes overdue/summary/pending_approvals/today/help/unknown · [JWT] Wave-2 /nl-query seam preserved · NO LLM · grep no fetch/anthropic/openai in resolver) · AI role-home "what needs you now" · NEW SIBLING role-home-engine (sole engine credit · CONSUMES insights-inbox-engine.buildInbox read-only · rule-ranked per (role × source_engine) weight matrix · 8 roles covered · honest empty when inbox empty · [JWT] Wave-2 /predict seam preserved · NO ML · greppable __reuse + READS_FROM) · RoleHomeFeed shared component used as web widget on /operix-go · honest banners mounted (DISHANI_RESOLVER_HONESTY + ROLE_HOME_HONESTY) · walls held: ask-dishani core 0-DIFF (DishaniContext/Panel/FloatingButton byte-identical · only the new context-set hook wired in ConditionalDishani) · insights-inbox-engine 0-DIFF (namespace import) · all 33 ERP card pages 0-DIFF · applications.ts 0-DIFF · MobileSmartInsightsPage 0-DIFF · hash-chain · retention · entitlements 0-DIFF · mobile-gap captures + camera/voice deferred to AM.2/Wave-2 · SP.4 flipped to 541b4b63 · 114→115 ⭐ · headSha TBD_AT_BANK
  {
    sprintNumber: 'AM1' as unknown as number, code: 'T-AM1-AI-Everywhere', composite: false, grade: 'A',
    headSha: '3fd7e61f', predecessorSha: '541b4b63', loc: 1000,
    newSiblings: ['role-home-engine'],
    bankDate: '2026-06-09', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Sprint RPT-6b T-RPT6b-RequestX-StoreHub-Reports · Reporting Arc · Phase C (Ops Hub-2, cohort b) · 12 pages chart-enabled across RequestX (7) + Store Hub (5) — 5 toggle recipe (AgeingPendingIndents · DepartmentWiseSummary · POAgainstIndent · IndentClosed · IndentPending · CycleCountStatus) and 7 dashboard recipe (IndentRegister · ServiceRequestRegister · StockIssueRegister · StockReceiptAckRegister · DepartmentConsumptionSummary · StockMovementRegister) · 12 layer-tagged KPI seeds (rq-indent · rq-ageing · rq-dept-summary · rq-po-against · rq-closed · rq-pending · rq-extra · st-issue · st-dept-consumption · st-receipt-ack · st-cycle-count · st-movement) · 4 DSC sources (requestx.indents · requestx.po-conversion · storehub.issues · storehub.movement) read-only wrappers of materialIndentsKey/serviceRequestsKey/capitalIndentsKey/stockIssuesKey/stockReceiptAcksKey · 1 legacy-recharts page EXCLUDED 0-DIFF (CategoryWiseSpendEstimate) · ServiceRequestRegister chosen as 7th RequestX page (CategoryWiseSpendEstimate is the legacy-recharts exclusion) · ScorecardTile OMITTED across all 12 pages — no page exposes a real bounded summary-% (status mixes/counts only) · hooks at top level · NO recharts import added · layouts PRESERVED (additive Card after existing grid/table) · honest empty-state · ZERO new SIBLINGs · headSha TBD_AT_BANK
  {
    ...
    headSha: '3b25c2e', predecessorSha: 'f5e1d93', loc: 980,
    newSiblings: [],
    bankDate: '2026-06-11', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Sprint RPT-7a T-RPT7a-SalesX-Registers · Reporting Arc · Phase C (Sales Hub, cohort a) · 11 SalesX register pages chart-enabled via dashboard recipe (ReportChart + integrity badge) — SalesOrderRegister · CustomerOrderRegister · QuotationRegisterV2 · SOMRegister · SRMRegister · DOMRegister · CustomerVoucherRegister · SalesReturnMemoRegister · CommissionRegister · FollowUpRegisterReport · SecondarySalesRegister · 11 layer-tagged KPI seeds (sx-sales-orders · sx-customer-orders · sx-quotations · sx-som · sx-srm · sx-dom · sx-vouchers · sx-returns · sx-commission · sx-followups · sx-secondary) · 2 SalesX DSC sources (salesx.orders wrapping ordersKey filtered to Sales Order · salesx.pipeline wrapping quotationsKey) — read-only · ScorecardTile OMITTED across all 11 pages · SalesXAnalytics + CampaignPerformanceReport (legacy-recharts) and the 6 RPT-7b analytical pages left 0-DIFF · headSha BACKFILLED 056371b at RPT-7b Block 4 · forward-extended in RPT-7b
  {
    sprintNumber: 'RPT7a' as unknown as number, code: 'T-RPT7a-SalesX-Registers', composite: false, grade: 'A',
    headSha: '056371b', predecessorSha: 'bf838cf', loc: 980,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-7b T-RPT7b-SalesX-Analytical-ProjX · Reporting Arc · Phase C (Sales Hub cohort b + ProjX) · 12 pages chart-enabled across SalesX analytical (6) + ProjX (7−1 excluded) — TOGGLE recipe (8 pages with shadcn <Table>): TargetVsAchievement (21) · PipelineSummary (15) · CoverageReport (22) · BeatProductivityReport (21) · SecondarySalesReport (35) · CrossDeptHandoffTracker (31) · ProjectRegister (15) · ProjxDocumentsRegister (19) — DASHBOARD recipe (4 pages with 0 shadcn <Table>): ProjectPnLReport · ProjectMarginReport · MilestoneStatusReport · ResourceUtilizationReport — EXCLUDED 0-DIFF: CashFlowProjectionReport (legacy-recharts) · 13 layer-tagged KPI seeds · 2 ProjX DSC sources · ScorecardTile OMITTED · ZERO new SIBLINGs · headSha BACKFILLED e250a8c at RPT-7c Block 5 · forward-extended in RPT-7c
  {
    sprintNumber: 'RPT7b' as unknown as number, code: 'T-RPT7b-SalesX-Analytical-ProjX', composite: false, grade: 'A',
    headSha: 'e250a8c', predecessorSha: '056371b', loc: 1080,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-7c T-RPT7c-Distributor-Customer-EcomX · Reporting Arc · Phase C (Sales Hub final cohort — closes the hub) · 10 pages chart-enabled across Distributor (3 of 5; 2 excluded legacy-recharts) + Customer (3 of 4; 1 excluded legacy-recharts) + EcomX (4) — TOGGLE recipe (1 page with shadcn <Table>): DistributorDemandForecastFeed (19) — DASHBOARD recipe (9 pages with 0 shadcn <Table>): DistributorOrderRegister (0) · CreditUtilReport (0) · CLVRankingsReport (0) · ChurnRiskReport (0) · SocialProofReport (0) · EcomXOrdersPage (0) · EcomXClaimsPage (0) · EcomXReturnsPage (0) · EcomXDashboardPage (0) — EXCLUDED 0-DIFF (legacy-recharts): DisputeStatsReport · SchemeEffectivenessReport · EngagementReport (distributor non-target) · LoyaltyPerformanceReport · WebStoreX skipped per honest study · 13 layer-tagged KPI seeds · 3 DSC sources · ScorecardTile OMITTED across all 10 pages · ZERO new SIBLINGs · headSha BACKFILLED 772c191 at RPT-8a Block 4 · forward-extended in RPT-8a
  {
    sprintNumber: 'RPT7c' as unknown as number, code: 'T-RPT7c-Distributor-Customer-EcomX', composite: false, grade: 'A',
    headSha: '772c191', predecessorSha: 'e250a8c', loc: 950,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-8a T-RPT8a-ServiceDesk-Dispatch · Reporting Arc · Phase C (Support+Pay+Dispatch hub, cohort a) · 12 pages chart-enabled across ServiceDesk (5 of 7; 2 excluded legacy-recharts) + Dispatch (7 of 8; 1 excluded legacy-recharts) — TOGGLE recipe (2 pages with shadcn <Table>): StockHoldReport (2) · DemoSerialRegister (1) — DASHBOARD recipe (10 pages with 0 shadcn <Table>): AMCRenewalForecastDrillDown (0) · ServiceAvailedTracker (0) · CustomerSLAEnquiry (0) · CustomerCommLog (0) · FutureTaskRegisterViewer (0) · InwardReceiptRegister (0) · LRTracker (0) · PackingSlipRegister (0) · DispatchReceiptRegister (0) · DispatchAnalytics (0) — EXCLUDED 0-DIFF (legacy-recharts): CustomerPnLReport (sd-customer-pnl seeded for RPT-12) · PromisedVsActualVariance (sd-promise-variance seeded for RPT-12) · TransporterScorecard (dp-transporter seeded for RPT-12) · ServiceDayBook (registry-served · RPT-3a) · 15 layer-tagged KPI seeds (sd-customer-pnl · sd-promise-variance · sd-amc-forecast · sd-service-availed · sd-sla[thresholds OMITTED — no real met-%] · sd-comm-log · sd-future-tasks · dp-stock-hold · dp-inward · dp-lr · dp-packing · dp-demo-serial · dp-transporter · dp-receipts · dp-analytics) · 4 DSC sources (servicedesk.tickets wrapping serviceTicketKey · servicedesk.amc wrapping amcRecordKey · dispatch.shipments wrapping dispatchReceiptsKey · dispatch.inward wrapping inwardReceiptsKey) · ScorecardTile OMITTED across all 12 pages · hooks at top level · NO recharts import added · layouts PRESERVED (additive Card after existing content) · ZERO new SIBLINGs · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT8a' as unknown as number, code: 'T-RPT8a-ServiceDesk-Dispatch', composite: false, grade: 'A',
    headSha: '25862b3', predecessorSha: '772c191', loc: 980,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-8b T-RPT8b-SmallCards-PhaseC-Close · Reporting Arc · Phase C final cohort (closes hub rollouts) · 6 pages chart-enabled across FrontDesk (1) + TaskFlow (1) + DocVault (3) + Pay-hub (1) — TOGGLE recipe (5 pages with shadcn <Table>): AddressBookReportPage (1) · SLAManagementPage (1) · DocumentRegister (1) · ExpiryReviewPage (2) · StatutoryReturns (6+) — DASHBOARD recipe (1 page with 0 shadcn <Table>): DrawingRegisterTree (0) — 6 layer-tagged KPI seeds (fd-addressbook · tf-sla · dv-documents · dv-drawings · dv-expiry · ph-statutory) · 4 DSC sources (frontdesk.contacts wrapping fdPartyContactsKey · taskflow.tasks wrapping tfSLAKey · docvault.documents wrapping erp_documents_{ec} · payhub.payroll wrapping payrollRunsKey — SAME storage the pay-hub workflow modules already read) · pay-hub workflow modules (Onboarding/PayrollProcessing/ExitAndFnF/etc.) + PayHubDayBook + TaskFlow boards + DocVault config pages STAY 0-DIFF by design · ScorecardTile OMITTED across all 6 pages — no real bounded summary-% available · hooks at top level · NO recharts import added · layouts PRESERVED (additive Card after existing content) · ZERO new SIBLINGs · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT8b' as unknown as number, code: 'T-RPT8b-SmallCards-PhaseC-Close', composite: false, grade: 'A',
    headSha: '862f0c3', predecessorSha: '25862b3', loc: 420,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Sprint RPT-9e T-RPT9e-Builder-Rollout-Support · Reporting Arc · Phase D builder rollout 4 of 4 · CLOSES RPT-9 · MECHANICAL ADDITIVE · mount the frozen <ReportBuilder cardId="…" /> from RPT-9a into 6 Support cards (frontdesk · servicedesk · taskflow · docvault · pay-hub→peoplepay · dispatch→dispatch-hub) mirroring the RPT-9b/9c/9d recipe · Canonical ids honoured: pay-hub→peoplepay · dispatch→dispatch-hub · ZERO new SIBLINGs · ≥30 cards now carry the builder · headSha BACKFILLED be771e8 at RPT-10a Block 5
  {
    sprintNumber: 'RPT9e' as unknown as number, code: 'T-RPT9e-Builder-Rollout-Support', composite: false, grade: 'A',
    headSha: 'be771e8', predecessorSha: '2b62547', loc: 210,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-10a T-RPT10a-Executive-Cockpits · Reporting Arc · Phase D cockpits 1 of 2 · 3 executive cockpit pages composing FROZEN primitives + Fin-hub DSC repair · Block 1: 4 NEW DSC sources (receivx.ar · payout.payments · billpassing.bills · eximx.shipments) wrapping existing storage/engines (outstandingKey · vendorPaymentBatchKey · billPassingKey · loadShippingBills) — un-empties the 4 Fin-hub Report Builder previews · dsc-card-id-integrity.test.ts extended (every mounted-builder card resolves ≥1 source · meta cards 'cc' + 'ix' excluded as documented) · Block 2: PromoterCockpitPage at /erp/command-center/promoter (TV mode · 4 auto-cycling sections @ 12s) · Block 3: CreditXRayPage in ReceivX (rx-credit-xray module · sidebar entry) · Block 4: SpendFunnelPage in Procure360 (spend-funnel module · sidebar entry · funnel by status — only stages with real data render · missing stages declared not faked) · 5 KPI seeds (xc-promoter · rx-credit-exposure · rx-overdue-pct · pr-spend-funnel · pr-vendor-concentration · idempotent · seed data only) · ZERO new SIBLINGs · walls held: ReportBuilder + report-builder-engine + report-definitions FROZEN · all banked pages incl. OEEDashboard + Comply360 audit pages 0-DIFF · headSha BACKFILLED 9b98b3c at RPT-10b Block 5
  {
    sprintNumber: 'RPT10a' as unknown as number, code: 'T-RPT10a-Executive-Cockpits', composite: false, grade: 'A',
    headSha: '9b98b3c', predecessorSha: 'be771e8', loc: 720,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-10b T-RPT10b-Ops-Cockpits-Audit-Upgrades · Reporting Arc · Phase D cockpits 2 of 2 · CLOSES RPT-10 (last new-build sprint of the arc) · 3 ops cockpit pages composing FROZEN primitives + additive dashboard recipe on 2 EXISTING Comply360 audit dashboards · Block 1: OEEBoardPage in Production (`prod-oee-board` module · sidebar entry) · consumes computeOEE from src/lib/oee-engine.ts with SAME wiring as banked OEEDashboard (factories + machines + job-cards + DWR → buildOEESourceData → computeOEE) · legacy OEEDashboard 0-DIFF · per-line OEE columns + A/P/Q stacked-column + per-line RAG tiles via resolveRag against prod-line-oee thresholds (RPT-6a seed) + integrity badge · honest empty-state · Block 2: COQPage in QualiCheck (`qc-coq` module · sidebar entry) · reads qualicheck.inspections + qualicheck.ncr · rejection-volume-by-source (qty_affected — rows carry no monetary cost) · NCR status×severity stacked-column · pass-rate RAG tile ONLY if a real pass-% derives · Prevention/Appraisal/Failure split DECLARED ABSENT (no cost-category fields on source rows · never invented) · Block 3: EVMPage in ProjX (`projx-evm` module · sidebar entry · projx-rpt-report-builder pattern mirror) · PV from current_contract_value · EV from billed_to_date · AC leg DECLARED ABSENT (no AC field on projx.projects / projx.financials rows) · SPI=EV/PV computed when PV>0 · CPI DECLARED ABSENT (AC missing) · S-curve line chart of cumulative milestone invoice_amount over target_date · Block 4: AuditFrameworkDashboardPage additive recipe section (rpt10b-audit-fw-section · ScorecardTile overall score + sub-score breakdown ReportChart + signReport integrity badge · existing 904L layout PRESERVED) · CostAuditDashboardPage additive cmp-cost-audit mgmt ScorecardTile (already had full recipe from prior sprint — additive tile so the new seed is rendered not orphan · existing layout PRESERVED) · pre-flight recharts grep returned 0/0 on both audit pages — neither excluded · Block 5: 5 KPI seeds (qc-coq · px-spi · px-cpi · cmp-audit-fw · cmp-cost-audit · idempotent · only rendered KPIs seeded) + sprint-history backfill RPT-10a headSha to 9b98b3c + self-seed RPT-10b · ZERO new SIBLINGs (cockpits compose oee-engine + DSC + frozen primitives · KPI seeds are data) · walls held: oee-engine.ts + production-engine.ts CONSUMED 0-DIFF · legacy OEEDashboard 0-DIFF · ReportBuilder/engine/definitions FROZEN · data-sources.ts 0-DIFF · all banked pages 0-DIFF · src/test/setup.ts 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT10b' as unknown as number, code: 'T-RPT10b-Ops-Cockpits-Audit-Upgrades', composite: false, grade: 'A',
    headSha: '415c52a', predecessorSha: '9b98b3c', loc: 920,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-12a T-RPT12a-BuilderUX-Close · Reporting Arc · Phase D arc-close 1 of 3 · six small closing capabilities composing the FROZEN engine layer · Block 1: PivotMatrix.tsx (rows×cols matrix + row/col/grand totals · UI only · engine already multi-dim) + Pivot preview tab on ReportBuilder when spec.groupBy.length===2 · Block 2: export-csv.ts (React-free toCsv + downloadCsv RFC4180-quoted · ONLY DOM via Blob link) + CSV buttons on ReportBuilder preview + 6 RPT-10 cockpits (CreditXRay/SpendFunnel/OEEBoard/COQ/EVM/PromoterCockpit) acting on REAL rows already in state · Block 3: report-search-entries.ts (one CommandEntry per registered DSC source + seeded KPI + saved-def · idempotent registerReportSearchEntries() pushes into BASE_COMMANDS via the registry's exported CommandEntry type · main.tsx wires init · command-palette-registry.ts itself 0-DIFF) · Block 4: intent-match.ts (client-side fuzzy · verb map sum/count/avg/min/max/top · NO LLM NO network · prefills ReportBuilder pickers · honest no-match line · user confirms — never auto-runs) · Block 5: narrative.ts (NarrativeProvider interface + registerNarrativeProvider + DEFAULT rule-based provider where every number derives from computeNarrativeAggregates over the real rows · no free text · LLM provider plugs in at Wave-2) · narrative line rendered in ReportBuilder preview + PromoterCockpit cash-ar section · Block 6: 10 forward-seed KPI dataSource pointers verified to resolve via getSource — xc-cash-position→reg:fc-ledger (existing FinCore ledger register · cash-bearing) + xc-promoter/px-cashflow/px-cpi/db-scheme-eff/db-disputes/cu-loyalty/sd-customer-pnl/sd-promise-variance/dp-transporter all resolve · ZERO stale pointers · ZERO new SIBLINGs (all 5 modules are utils/components on the frozen engine) · walls held: report-builder-engine.ts + report-definitions.ts + command-palette-registry.ts + data-sources.ts + src/test/setup.ts 0-DIFF · all banked pages 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT12a' as unknown as number, code: 'T-RPT12a-BuilderUX-Close', composite: false, grade: 'A',
    headSha: '22d6860', predecessorSha: '415c52a', loc: 780,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-12b T-RPT12b-Migration-Wave1 · Reporting Arc · Phase D arc-close 2 of 3 · MECHANICAL CHART-LAYER SWAP · migrate 15 legacy pages off raw recharts onto the frozen ReportChart framework · Production 9 (OEEDashboard · CapacityPlanningDashboard · PlanActualRolling · ManpowerProductionReport · ShiftwiseProductionReport · WastageDashboard · CarbonAwareProductionPlanner · ProductionCarbonDashboard · JobWorkAgeingAnalysis) · Dispatch 4 (SavingsROIDashboard · TransporterScorecard · ReconciliationSummaryReport · PackingConsumptionReport) · SalesX 2 (SalesXAnalytics · CampaignPerformanceReport) · per page: removed recharts import · replaced each <ResponsiveContainer>...</ResponsiveContainer> block with <ReportChart data={…} config={defaultChartConfig({…})} /> over the SAME computed data the page already builds (zero data-logic change) · added signReport integrity badge + chart-host testid · pre-seeded KPI ids reused where present (prod-line-oee on OEEDashboard · dp-transporter on TransporterScorecard) · 13 new layer-tagged KPI seeds in kpi-registry.ts for the remaining surfaces · SalesXAnalytics funnel-onClick drill replaced by parallel stage chips (same drillStage state · same drillRows) · ONE TEST FILE PER PAGE (15 total) reading source · asserting zero recharts import · ReportChart present · signReport present · integrity badge testid present · closing assertion in campaign-performance test: 3-dir grep returns 0 · ZERO new SIBLINGs · ZERO engine edits · walls held: ReportBuilder + report-builder-engine + report-definitions + data-sources.ts + framework barrel 0-DIFF · all banked pages outside the 15 0-DIFF · src/test/setup.ts 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT12b' as unknown as number, code: 'T-RPT12b-Migration-Wave1', composite: false, grade: 'A',
    headSha: 'c5cee13', predecessorSha: '22d6860', loc: 1180,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint RPT-12c T-RPT12c-Migration-Wave2-Arc-Close · Reporting Arc · Phase D arc-close 3 of 3 · REPORTING_ARC: COMPLETE · MECHANICAL CHART-LAYER SWAP for the last 19 legacy pages · ServiceDesk 7 (RefurbishedUnitLifecycle · EngineerMarketplace · SLAPerformance · PromisedVsActualVariance · CustomerPnLReport · EngineerBurnoutDashboard · AMCRenewalForecast) · ProjX 1 (CashFlowProjectionReport 603L wiring px-cashflow at last) · Distributor 3 (SchemeEffectivenessReport wiring db-scheme-eff · EngagementReport · DisputeStatsReport wiring db-disputes) · PayOut 2 (VendorAnalytics · CashFlowDashboard) · Singles 6 (VendorScoringPanel · LoyaltyPerformanceReport wiring cu-loyalty · RateVarianceGraphPanel · ESGEnergyDashboard · SiteTwinDashboard · CategoryWiseSpendEstimate) · per page: removed recharts import · replaced each <ResponsiveContainer>...</ResponsiveContainer> block with <ReportChart data={…} config={defaultChartConfig({…})} /> over the SAME computed data the page already builds (zero data-logic change) · signReport integrity badge + chart-host testid · pre-seeded KPI ids wired (px-cashflow · sd-customer-pnl · sd-promise-variance · db-scheme-eff · db-disputes · cu-loyalty) · ONE TEST FILE PER PAGE (19 total) reading source asserting zero recharts + ReportChart + signReport + integrity testid + compute-preservation grep · NEW arc-close-sweep.test.ts (4 closing assertions: zero recharts in src/pages via fs-walk + RECHARTS_LEGACY_ALLOWLIST [src/pages/vendor-portal/VendorPerformanceView.tsx · non-ERP Phase-1.A stand-alone view · vendor-portal arc follow-up] · every ReportChart/TableChartToggle page references signReport · every KPI dataSource resolves via getSource or reg:* · 19-target migration grep) · sprint-history backfill RPT-12b headSha to c5cee13 + self-seed RPT-12c · ZERO new SIBLINGs · ZERO engine edits · ZERO data-source edits · walls held: ReportBuilder + report-builder-engine + report-definitions + data-sources.ts + framework barrel 0-DIFF · all banked pages outside the 19 0-DIFF · src/test/setup.ts 0-DIFF · REPORTING_ARC: COMPLETE (RPT-1a → RPT-12c · 30+ banked pages on framework · 5 cockpits · 30+ builder mounts · 33 invariant green · zero recharts in src/pages/erp) · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT12c' as unknown as number, code: 'T-RPT12c-Migration-Wave2-Arc-Close', composite: false, grade: 'A',
    headSha: 'bbc6ada', predecessorSha: 'c5cee13', loc: 1620,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-1 T-W1C1-DocSendBar-Reports · Wave-1 Close Arc · DocSendBar floor canon onto the reporting layer · ONE wrapper (ReportSendHeader · ≤80 LOC) composes the FROZEN DocSendBar · pre-computes the rule-based narrative line via describeReport (from real rows; never fabricated) and exposes it through sourceRecord so it flows through the standard communication-engine merge path · NO printPayload (PDF button uses DocSendBar's own honest "no payload available" toast) · ONE shared-component edit (TableChartToggle additive only · derives title from chartConfig.title || document.title || 'Report' · hideSend escape hatch default false) → blanket coverage of all 106 wrapped report pages with ZERO call-site edits · 10 manual cockpit/builder/daybook mounts (PromoterCockpit · CrossCardDayBook · CreditXRay · SpendFunnel · OEEBoard · COQ · EVM · AuditFramework · CostAudit · ReportBuilder preview) · honest channels preserved (mailto real · .eml real · PDF-absent honest toast · never fake a send) · ZERO new SIBLINGs (wrapper composes frozen primitives) · 0-DIFF: DocSendBar.tsx + communication-engine + whatsapp-channel-engine + export-csv.ts + narrative.ts + report-builder-engine.ts + all 106 wrapped pages outside the 10 manual mounts + src/test/setup.ts · headSha 9a47f31
  {
    sprintNumber: 'W1C1' as unknown as number, code: 'T-W1C1-DocSendBar-Reports', composite: false, grade: 'A',
    headSha: '9a47f31', predecessorSha: 'bbc6ada', loc: 80,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-2 T-W1C2-Polish-Riders · Wave-1 Close Arc · 5-block polish bundle + audit riders · Block 1 InlineMasterCreate REAL FORMS (PartyPicker + LedgerPicker · the 4 Polish-1.5 toast TODOs removed · minimal Dialog forms — party: name/type/GSTIN-optional · ledger: name/group — writing through the SAME master storage (erp_group_customer_master / erp_group_vendor_master / erp_group_ledger_definitions) the full Customer/Vendor/Ledger Master pages persist into · new record auto-selected on save) · Block 2 ItemPicker (StockTransferLineGrid · plain-text Item Input swapped for new src/components/fincore/pickers/ItemPicker.tsx reading erp_inventory_items · qty/UOM/rate flow preserved) · Block 3 RIDER VendorPerformanceView migration (recharts → ReportChart bar over factor_breakdown · integrity badge wired · removed from RECHARTS_LEGACY_ALLOWLIST) · Block 4 RIDER SiteTwin honest-data pass (hardcoded trend sparklines REMOVED) · Block 5 RIDER sprint-81a relaxation + sprint-history backfill W1C-1 + self-seed W1C-2 · ZERO new SIBLINGs · headSha 64a81ff
  {
    sprintNumber: 'W1C2' as unknown as number, code: 'T-W1C2-Polish-Riders', composite: false, grade: 'A',
    headSha: '64a81ff', predecessorSha: '9a47f31', loc: 720,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-3 T-W1C3-ComingSoon-WelcomeTruth · Wave-1 Close Arc · sprint 3 · coming-soon census close + welcome WIP-badge truth pass · Block 1 CompaniesEntitiesPage replaced (now lists real companies from `erp_companies` foundation store with per-row compliance chips (status · GST regs count · LUT bonds count · city/state) linking to /erp/foundation/companies/:id/edit · honest empty-state CTA; 3 compliance sub-tabs Schedule-M/CARO-Ext/CFR-Part-11 preserved to keep S77b shell contract) · Block 2 hub placeholder tiles wire-or-remove pass: AccountingHub stale "Coming Soon" tiles REMOVED (Capital Assets · Chart of Accounts · Cost Centres · Budget Master — no live routes post-arc) · 6 hub fallback panels reworded "Coming Soon" → "Module not yet wired" (CustomerHubPage · DistributorHubPage · SalesXPage · PayHubPage · DispatchOpsPage · DispatchHubPage — these are default render-case fallbacks for unmapped sidebar entries) · erp/Dashboard toast + badge reworded honestly to "arrives with Wave-2" (no app currently carries status:'coming_soon' so this is defensive) · CalendarPage comment "Coming Soon since S69" → "live since Sprint 78b" (comment-only tidy) · Block 3 Welcome.tsx truth pass: ALL 10 wip badges lifted (Operix Udyam Kendra dashboard included — Block 2 left it fully honest/wired, no Wave-2 toast actually fires); ADDED welcome-badge-truth.test.tsx permanent guard that every remaining wip badge target must contain a genuine WIP marker · Block 4 sprint-history backfill W1C-2 headSha → 64a81ff + self-seed W1C-3 · 4 test files (companies-entities · hub-placeholders · welcome-badge-truth · institutional) · ZERO new SIBLINGs · 0-DIFF: PDFInvoiceUpload (honest scanned-PDF error) · CompanyForm/ParentCompany Hindi (Coming Soon) labels (Wave-2 i18n) · customer/Orders + customer/Profile + Profile + tower/Security + vendor-portal/VendorDashboard (Wave-2 auth-gated) · foundation/company stores (consumed read-only) · 77b ScheduleMPage/CAROExtendedPage/CFRPart11DeeplinkPage (preserved as sub-tabs) · headSha TBD_AT_BANK
  {
    sprintNumber: 'W1C3' as unknown as number, code: 'T-W1C3-ComingSoon-WelcomeTruth', composite: false, grade: 'A',
    headSha: 'f6d8bcb', predecessorSha: '64a81ff', loc: 220,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-4 T-W1C4-AutoSend-TierL · Wave-1 Close Arc · sprint 4 · Comm Pillar open item #1 · Block 1 auto-send-rules-engine.ts (CC-editable AutoSendRule rows under sole namespaced key autoSendRulesKey · evaluateAutoSend orchestration only: match → resolveRule (party/department/fixed with targetUserId mail-profile fallback) → renderTemplate (existing TemplateMaster) → enqueue on existing outbox delivery_mode=queued_for_wave2 · ZERO direct send code · grep-lock no mailto/fetch/smtp in engine · starter rules SEEDED DISABLED for the TWO real P8.2 spine events that also have TemplateMaster rows: approval.pending → tpl-approval · digest.my_reminders → tpl-reminders · STARTER_RULES_UNAVAILABLE exports the 5 prompt-suggested events NOT in spine: voucher-posted · po-approved · dln-created · sla-breach · payment-released — honestly NOT seeded · handleNotificationEvent additive adapter for spine fan-out · recentEnqueuesForRule per-rule last-5 from outbox) · Block 2 AutoSendRulesPage CC admin (Command Center governance group · rule CRUD enable/toggle/template-picker/recipient-resolver/sender-class · per-rule last-5 enqueue log) · CommandCenterPage module union + KNOWN_MODULES + switch case + sidebar entry (additive — communication-console 0-DIFF · governance siblings 0-DIFF) · Block 3 sibling-register: ONE NEW SIBLING auto-send-rules-engine (Wave-1 Close Arc's first sibling · justified: genuinely new orchestration engine) · sprint-history backfill W1C-3 headSha → f6d8bcb + self-seed W1C-4 · 5 test files (rule-crud · evaluate-auto-send · no-direct-send-grep · starter-rules-honest · institutional) · 0-DIFF: communication-engine (renderTemplate/resolveRecipients/listDepartmentEmails/listUserMailProfiles/getCompanyMailSettings/listOutbox consumed) · types/communication.ts (outboxKey/OutboxMessage shapes consumed) · notification-engine (additive subscribe only via handleNotificationEvent · spine 0-DIFF) · DocSendBar · all banked pages · headSha TBD_AT_BANK
  {
    sprintNumber: 'W1C4' as unknown as number, code: 'T-W1C4-AutoSend-TierL', composite: false, grade: 'A',
    headSha: 'da3dd3b', predecessorSha: 'f6d8bcb', loc: 480,
    newSiblings: ['auto-send-rules-engine'],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint M1 T-M1-MobileARC-Close · the FINAL build sprint of Wave-1 · Mobile-ARC close · Block 1 Transporter PWA under src/pages/mobile/transporter/ (Home worklist with aging chips + LR queue with record view + ManifestAck calling wms-manifest-engine.acknowledgeManifest writing the SAME ManifestAck.acknowledged_by/ack_at/discrepancy_note fields the desktop W3 seam writes + POD Capture with photo capture-input + e-sign canvas + honest pending_sync queue via PODStatus 'pending' on podsKey + read-only Disputes from freight disputesKey + read-only Payments from transporterInvoicesKey) · Block 2 Vendor PWA under src/pages/mobile/vendor/ (Home worklist + POAck appending PoFollowup outcome='committed' on purchaseOrdersKey + ASNCreate appending PoFollowup outcome='partial' with LR/dispatch/vehicle/packages structured notes + InvoiceSubmit writing BillPassingRecord status='pending_match' into billPassingKey — the 3-way-match intake the desktop reads + read-only Payments from vendorPaymentBatchKey + Documents read+upload toggling VendorDocumentRequest.status='submitted' with submitted_at) · Block 3 DocSendBar-mobile floor mounting the FROZEN ReportSendHeader on MobileUniversalReportPage + MobileLRQueuePage record view + MobilePODCapturePage confirmation + MobilePOAckPage record view + MobileVendorInvoiceSubmitPage confirmation + MobileCustomerOrdersPage (Factory's stated View+Send case) · Block 4 institutional: sprint-history backfill W1C-4 headSha→da3dd3b + self-seed M1 · ZERO new SIBLINGs (consumed lrAcceptancesKey · manifestsKey · manifestAcksKey · acknowledgeManifest · podsKey · disputesKey · transporterInvoicesKey · purchaseOrdersKey · billPassingKey · vendorPaymentBatchKey · vendorDocumentRequestKey · ReportSendHeader · DocSendBar — all engines + stores 0-DIFF) · launcher tiles for transporter + vendor on OperixGoPage with the standard Wave-2 chip (external-persona built-now-opened-Wave-2 mirroring distributor/customer pattern) · MobileRouter additions: 6 transporter routes + 6 vendor routes + 12 imports · 3 test files (transporter · vendor · send-floor) · headSha da3dd3b
  {
    sprintNumber: 'M1' as unknown as number, code: 'T-M1-MobileARC-Close', composite: false, grade: 'A',
    headSha: 'da3dd3b', predecessorSha: 'da3dd3b', loc: 1450,
    newSiblings: [],
    bankDate: '2026-06-12', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Sprint W1C-6 T-W1C6-First-Run-Seed · Wave-1 Close Arc · sprint 6 · LEAN onboarding pass to clear the smoke-test blocker (fresh boot lands every financial card behind SelectCompanyGate because zero companies are seeded) · Block 1 SelectCompanyGate empty-state CTA: Load demo company + sample data → useDemoSeedLoader.loadModule('foundation','all') → reload (re-evaluates entities.length); secondary Quick add company opens QuickCreateEntityDialog; manual Command-Center link preserved · Block 2 Dashboard.tsx FirstRunOnboardingBanner above the 33-card grid · hidden when erp_group_entities populated · dismissible via erp_first_run_banner_dismissed · Block 3 createMinimalEntity(name, gstin?, state?) added to existing foundation engine entity-setup-service.ts (writes erp_group_entities + erp_companies + seeds erp_parent_company only if absent) · QuickCreateEntityDialog consumes it · 7-step CompanyForm.tsx stays 0-DIFF · Block 4 sprint-history backfill W1C-5 headSha → b710a79 + self-seed W1C-6 · 4 test files (gate-load-demo-cta · onboarding-banner-conditional · quick-create-minimal · institutional) · ZERO new SIBLINGs (useDemoSeedLoader + EntitySetupDialog + CompanyForm all CONSUMED 0-DIFF · createMinimalEntity is an added function on the EXISTING foundation engine) · walls held: useDemoSeedLoader.ts 0-DIFF · EntitySetupDialog.tsx 0-DIFF · CompanyForm.tsx 0-DIFF · all 49 gate-using surfaces 0-DIFF · all banked pages 0-DIFF · headSha TBD_AT_BANK
  {
    ...
    headSha: '64dced9', predecessorSha: 'b710a79', loc: 280,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-7a T-W1C7a-CC-Config-Seed · Wave-1 Close Arc · demo-seed sprint 1 of 3 · activates the Command Center governance layer so a loaded demo entity presents as configured · Block 1 cc-config-seed.ts seed writers (consume DEFAULT_GROUP_CONFIG / DEFAULT_RCM / DEFAULT_SETTLEMENT / DEFAULT_OUTSTANDING / DEFAULT_LC schemas bytes-identical · group flags flipped ON · ledger-mapping fields kept HONEST blank since fincore seed does not pre-create RCM/LC ledger ids · summary marker erp_comply360_config_${entityCode} writes Comply360ConfigSummary so OverviewModule.isConfigured() flips true) · Block 2 auto-send rule activation per demo entity via the engine's existing upsertAutoSendRule path (approval.pending + digest.my_reminders → enabled:true · engine 0-DIFF · real entities untouched) · Block 3 SecurityModule.IntegrationsPanel config-driven via loadIntegrationsForEntity (ruling b · demo entities → connected set · fresh real entity → ALL not_configured · NO fake connected) · Block 4 sprint-history backfill W1C-6 headSha → 64dced9 + self-seed W1C-7a · 4 test files (comply360-config-seed · auto-send-rules-enabled · integrations-config-driven · institutional with post-seed coverage assertion) · ZERO new SIBLINGs (cc-config-seed.ts is a seed-data file + read-helper · NOT an engine) · walls held: ComplianceSettingsAutomation.tsx 0-DIFF · auto-send-rules-engine.ts 0-DIFF · comply360 engines 0-DIFF · all banked pages 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'W1C7a' as unknown as number, code: 'T-W1C7a-CC-Config-Seed', composite: false, grade: 'A',
    headSha: '34ae6f8', predecessorSha: '64dced9', loc: 260,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-7b T-W1C7b-Demo-Txns-Finance · Wave-1 Close Arc · demo-seed sprint 2 of 3 · financial/procurement TRANSACTIONS round-trippable through real read paths · 6 card-domains seeded referentially-consistent · PayOut (vendorPaymentBatchKey · 2 batches · 1 released referencing demo bp-1 · 1 queued referencing demo bp-2 · plus vendorAdvancesKey with 1 fully-adjusted advance against po-1) · BillPassing (billPassingKey · 2 records · 1 approved_for_fcpi 3-way-clean against po-1+grn-1 · 1 matched_with_variance against po-2) · EximX (ebrcKey + edpmsKey · 1 issued eBRC + 1 closed EDPMS realisation pair) · Procure360 (purchaseOrdersKey · 2 POs · 1 fully_received feeding the chain · 1 approved · plus grnsKey with 1 posted GRN against po-1) · Comply360 (calls existing comply360-demo-seed-engine.applyDemoSeed for NBFC loans + RERA projects + AI ROI rows · marker-guarded idempotent) · Vendor-Portal (vendorActivityKey · 3 activities) · ZERO new engines/SIBLINGs (only new file is src/data/demo-transactions-finance-procurement.ts which is pure seed-data + registered as a new DEMO_MODULE in useDemoSeedLoader) · every row id prefixed demo-w1c7b- so purgeDemoData clears the cluster · sprint-history backfill W1C-7a headSha → 34ae6f8 + self-seed W1C-7b · 6 test files (one per card-domain) asserting key populates AND a read through the real engine returns the rows · walls held: bill-passing-engine 0-DIFF · po-management-engine 0-DIFF · comply360 engines 0-DIFF · ComplianceSettingsAutomation 0-DIFF · 7a config seed 0-DIFF · all card pages 0-DIFF · all banked surfaces 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'W1C7b' as unknown as number, code: 'T-W1C7b-Demo-Txns-Finance', composite: false, grade: 'A',
    headSha: '28507ed', predecessorSha: '34ae6f8', loc: 420,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-7c T-W1C7c-Demo-Txns-Ops-Close · Wave-1 Close Arc · demo-seed sprint 3 of 3 · CLOSES full-demo capability · ops/support TRANSACTIONS round-trippable through canonical register keys · 11 card-domains: QualiCheck (qaInspectionKey + ncrKey + capaKey · 1 pass-with-reject inspection + 1 minor NCR + 1 actions-assigned CAPA referentially linked to W1C-7b GRN/PO) · Dispatch (dispatchReceiptsKey · 1 delivered DR with POD against the DEMO_DELIVERY_MEMOS dm-demo-1 seeded by the orchestrator) · Logistics (logisticActivityKey + lrAcceptancesKey · LR accepted + POD uploaded activities + 1 LR acceptance) · ProjX (project_milestones_${e} · 2 milestones · 1 completed + 1 in_progress with earned-value for EVM cockpit) · ServiceDesk (serviceTicketKey + amcRecordKey · 1 in_progress ticket linked to 1 active AMC) · DocVault (erp_documents_${e} · 2 docs incl 1 contract + 1 engineering drawing — same store EngineeringX drawings consume) · EngineeringX (bomKey · 1 active BOM with 2 components feeding the BOM register · drawings via DocVault store) · MaintainPro (equipmentKey + workOrderKey · 1 running equipment + 1 in_progress PM work order) · RequestX (materialIndentsKey · 1 submitted indent for the default entity scope) · ZERO new engines/SIBLINGs (single new file src/data/demo-transactions-ops-close.ts · pure seed-data + registered as DEMO_MODULE ops-close-txns in useDemoSeedLoader) · every row id prefixed demo-w1c7c- so purgeDemoData clears the cluster · sprint-history backfill W1C-7b headSha → 28507ed + self-seed W1C-7c · per-card tests + full-demo-coverage capstone asserting populated primary register key for the 33-card roster (GateFlow + WebStoreX declared by-design-empty Wave-2 deferrals) · walls held: 7a config seed 0-DIFF · 7b finance seed 0-DIFF · every card engine + page 0-DIFF · all banked surfaces 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'W1C7c' as unknown as number, code: 'T-W1C7c-Demo-Txns-Ops-Close', composite: false, grade: 'A',
    headSha: '8ee1757', predecessorSha: '28507ed', loc: 480,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint W1C-8 T-W1C8-SigmaFlow-Scenario · Wave-1 Close Arc · 8th /welcome/scenarios blueprint · adds SigmaFlow Control India (Kolkata · DI valves + water-works fittings · ISO 9001 · BIS · British-Standard · Sigma Corp USA collaboration · PQRS mantra · ~74 employees · imports DI castings) as the 8th blueprint with a NEW additive 'valve-mfg' archetype value on DemoArchetype · DEMO_ITEMS_VALVE_MFG · 4 customers + 4 vendors · DEMO_BOM_VALVE_MFG · 7 existing blueprints + 3 existing archetypes 0-DIFF · ZERO new SIBLINGs · headSha 8caee7d
  {
    sprintNumber: 'W1C8' as unknown as number, code: 'T-W1C8-SigmaFlow-Scenario', composite: false, grade: 'A',
    headSha: '8caee7d', predecessorSha: '8ee1757', loc: 220,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Hotfix T-TowerLayout-Forced-Dark-Fix · root cause of "Control Tower stays dark when global toggle switches to light" — src/components/layout/TowerLayout.tsx line 52 hardcoded className="dark" on the wrapper div + inline style background:#0D1B2A (×2) + bg-[#0D1B2A] (×2) + a text-white/*+border-white/* palette built only for dark, so Tower re-applied .dark to its own subtree regardless of the global toggle. Other layouts (BridgeLayout/CustomerLayout) were already clean — TowerLayout was the sole offender. Fix removes the forced .dark + the two inline navy styles + the two bg-[#0D1B2A], and converts all ~18 hardcoded white/navy chrome classes to the semantic token vocabulary mirrored from BridgeLayout (bg-background/bg-card/border-border/text-foreground/text-muted-foreground/bg-accent/bg-muted). cyan logo + emerald status dot also tokenized to primary/success per project colour rule. ZERO new SIBLINGs · ZERO logic changes (nav/routing/collapse untouched) · touch TowerLayout.tsx + guard test tower-layout-theme.test asserting no className="dark" literal, no #0D1B2A, zero bg-[#/text-white/border-white chrome + token vocabulary present · predecessor 931ba7f
  {
    ...
    headSha: '5296b08', predecessorSha: '931ba7f', loc: 30,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Sprint W1C-10 T-W1C10-Smoke-Cleanup · Wave-1 Close Arc · closes all 5 smoke-test findings (F-1..F-5) · presentation/wiring only · ZERO logic-rewrite · F-1 CustomerLayout.tsx sidebar inline style hsl(222 47% 11%) + rgba(255,255,255,0.06) border converted to bg-card border-border (4th & final theme-layer · mirrors TowerLayout/BridgeLayout token vocabulary) · F-3 demo-seed-manifest.purgeDemoData augmented with belt-and-suspenders entity-suffix scan (`*_${entityCode}`) so purge clears keys even when seeders forgot to recordDemoKeys — round-trip seed→purge→assert-cleared now true · F-4 ProvisioningManager.refresh merges listProvisionRequests('demo-entity') + listProvisionRequests('public-build-your-plan') so BYP-submitted requests appear in the Tower queue (cross-scope inbox) · F-2 CustomerDashboard hardcoded RECENT_INVOICES/RECENT_ORDERS/MONTHLY_PURCHASES constants replaced by real reads of customerOrdersKey(SMRT) — derived invoices (status-mapped from order lifecycle · honest derivation comment) · derived monthly buckets · honest empty-state when no seeded orders (no synthetic rows · founder-rule held) · F-5 ClientBlueprintsPage line 453 "Seven design-partner" → CLIENT_BLUEPRINTS.length with sync comment (2nd instance · W1C-9 fixed the card-description) · ZERO new SIBLINGs · 4 new guard/behavioral tests: customer-layout-theme · demo-purge-roundtrip · provisioning-manager-byp-merge · customer-dashboard-real-reads · walls held: seed orchestrator write logic 0-DIFF (chose purge-scan path) · partner-portal-engine 0-DIFF (consumed pattern only) · all other layouts 0-DIFF · predecessor 0cfb411 · headSha TBD_AT_BANK
  {
    ...
    headSha: 'dcd42db', predecessorSha: '0cfb411', loc: 220,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint B1 T-B1-Abdos-Group-Seed · Wave-1 · turns the existing single-entity ABDOS blueprint into a real multi-company GROUP that feeds the Group Consolidation page (Consolidated P&L + TB + eliminations · DP-A3-9 scope-wall held: BS/CF/NCI/FX NOT surfaced) · NEW seed module src/data/demo-abdos-group.ts (consumed engines only · ZERO new SIBLINGs) · Block 1: 6 entities registered into erp_group_entities — ABDOS (parent · Abdos Group Holdings) + ABLSC (Life Sciences) + ABCMF (Contract Mfg) + ABPKG (Packaging) + ABDST (Distribution) + ABHHC (Hygiene & Homecare) · Block 2: 5 group-structure nodes via upsertGroupStructure with the Ind-AS METHOD MIX — ABLSC/ABCMF/ABPKG = subsidiary/100%/full · ABDST = joint_venture/60%/proportional · ABHHC = associate/30%/equity (exercises all 3 Ind-AS methods) · Block 3: per-entity posted vouchers seeded directly via vouchersKey(shortCode) so consolidate() has real numbers — revenue/COGS/opex/opening-cash by L3 codes SALE/PURCH/EMPB/CASH/TPAY/TREC (idempotent · skips if seeded already) · Block 4: 3 IC service_charge txns via createICTransaction + postICTransaction — ABPKG→ABCMF laminated tubes ₹2.5L · ABLSC→ABDST labware ₹1.8L · ABCMF→ABDOS management/royalty ₹1.2L · Block 5: ClientBlueprintsPage ABDOS blueprint subtitle/description updated to reflect the multi-entity group + handleLoadDemo wired to call seedAbdosGroup() additionally when entityCode==='ABDOS' (7 other blueprints 0-DIFF) · capstone test: consolidate({fy:'2024-25'}) returns entity_count≥4 + eliminations_applied≥1 + balanced=true · predecessor dcd42db · headSha TBD_AT_BANK
  {
    sprintNumber: 'B1' as unknown as number, code: 'T-B1-Abdos-Group-Seed', composite: false, grade: 'A',
    headSha: '947bd0c', predecessorSha: 'dcd42db', loc: 320,
    newSiblings: [],
    bankDate: '2026-06-13', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint CL-1 T-CL1-Cleanup-Foundation-Seed-Cosmetics · CLEANUP ARC sprint 1 of 3 · LEAN (~15 isolated files) · resolves smoke-test findings B2-F1/B3-F1/B4-F1/B16-F1/B16-F2 + dead-code + B11-F1 theme-guard · Block 1 B2-F1 foundation bridge (CompanyList/SubsidiaryList/BranchOfficeList read loadEntities() ⨝ listGroupStructure() instead of MOCK_* arrays · honest empty-state · OrgStructureHub seeded-group banner additive · ParentCompany hydrates legalEntityName/shortCode from parent entity as fallback when erp_parent_company is empty) · Block 2 B3-F1 orchestrator calls seedFinanceProcurementTxnsForDemo(entityCode) on the scenario seed path (every blueprint · not just DEFAULT) + B4-F1 SRM key already converged (supplyRequestMemosKey === orchestrator write key `erp_supply_request_memos_${e}` · test locks the contract round-trip) · Block 3 Mech4 EximX seedSinhaEximX(entityCode = 'sinha-trading') parameterized · 3 callers (EximXPage/EximXExportLayout/EximXImportLayout) pass useEntityCode() · canonical keys erp_${e}_iec / erp_${e}_lut · Mech5 RecentErrorsPage useState(getActiveEntity()) lazy-capture replaced by reactive useEntityCode() · local getActiveEntity helper removed · Block 4 B16-F1 <OfflineIndicator/> mounted on MobileShopFloorOperatorPage header · B16-F2 InstallPromptBanner mounted on Wave-1 /operix-go/* shell via OperixGoChrome route-aware sibling in App.tsx · dead-code MobilePODCapture (unrouted rich variant) deleted · MobilePODCapturePage (transporter live) kept · B11-F1 theme-guard test asserts zero bg-[# panel literals in src/pages/erp/** (expect PASS) · 55 text-white deliberately NOT converted (mostly intentional) · ZERO new SIBLINGs · OUT-OF-SCOPE (CL-2/CL-3 0-DIFF): ServiceDesk engine-signature defaults · 101 hardcoded-entity pages · 23 raw-key readers beyond RecentErrors · predecessor 947bd0c · headSha TBD_AT_BANK
  {
    sprintNumber: 'CL1' as unknown as number, code: 'T-CL1-Cleanup-Foundation-Seed-Cosmetics', composite: false, grade: 'A',
    headSha: '95c1990', predecessorSha: '947bd0c', loc: 220,
    newSiblings: [],
  ...
  // 🆕 Sprint CL-2b T-CL2b-ServiceDesk-EngineStrip · CLEANUP ARC sprint 2b of 3 · the BIG engine strip · applies the CL-2 proven pattern to servicedesk-engine.ts (75 ` = DEFAULT_ENTITY` defaults across ~21 exported functions) → param signatures become required `entity_id: string` · 2 functions reordered to satisfy TS1016 (decideAMCApplicability + transitionProposalStatus: entity_id moved before optional reason) · ~105 TSC errors surfaced across 21 files (the worklist) → all fixed by passing the real entity (useEntityCode() at component top-level for StandbyLoanList · AMCProposalList · AMCProposalDetail · ServiceTicketDetail · EngineerBurnoutDashboard · SparesIssuedFromField · CustomerPnLReport · AMCActiveList · AMCLapsedList · AMCExpiringList · AMCApplicabilityDecision · InstallationVerificationDetail · ServiceQuoteOptimizer · RepairRouteList · CallTypeMasterSettings · VoiceOfCustomerAggregation · MobileServiceCompletion · MobileServiceTicketRaise; ENTITY const threading for ServiceTicketRaise + servicedesk-bridges + approval-adapters; ticket.entity_id threading in ServiceTicketDetail handlers; tests pass 'OPRX' explicitly) · entity-isolation test: seed X=OPRX + Y=SMRT → assert X-call returns ONLY X's data · TSC back to 0 · ZERO new SIBLINGs · DEFAULT_ENTITY const retained only for body fallbacks in filter-based fns (not signature defaults) · predecessor e57f94d · headSha TBD_AT_BANK
  {
    sprintNumber: 'CL2b' as unknown as number, code: 'T-CL2b-ServiceDesk-EngineStrip', composite: false, grade: 'A',
    headSha: 'c7f7204', predecessorSha: 'e57f94d', loc: 320,
    newSiblings: [],
    bankDate: '2026-06-14', provenance: 'CONFIRMED',
  },
  ...
  // 🆕 Sprint CL-3b T-CL3b-MidClusters-HookSweep · CLEANUP ARC sprint 3b · hardcode→hook sweep · 4 mid clusters (customer-hub 12 + projx 11 + sitex 9 + distributor-hub 8 = 40 files) · removed module-scope `const ENTITY/entityCode/entity = DEFAULT_ENTITY_SHORTCODE` and replaced with `const { entityCode } = useEntityCode()` at component TOP LEVEL (sitex uses alias `{ entityCode: entity }`) · threaded `entityCode: string` through module-scope helpers that reference the entity (loadCustomers/loadOrders/readList/readSnapshots/etc.) and updated call sites · converted module-scope const data (SampleKits DEMO_TEMPLATES → buildDemoTemplates(entityCode), VoiceComplaintCapture COMPLAINTS_KEY/DISPUTES_KEY → key factories) · removed duplicate `entityCode` from useCardEntitlement() destructures (CustomerHubWelcome, SchemeSimulator) · added entityCode to useMemo/useEffect dep arrays · 4 NEW GUARD TESTS (one per cluster): src/__tests__/cl-3b/{customer-hub,projx,sitex,distributor-hub}-entity-hook-guard.test.ts mirror CL-3a guard, ROOT pointed at each cluster dir, assert offenders.toEqual([]) · TSC 0 · ESLint 0/0 (repo-wide) · Vitest 4/4 PASS guards · ZERO new SIBLINGs · NON-GOAL: vendor-portal/payout/distributor/masters/salesx/fincore/components → CL-3c/d · predecessor 4e2596d · headSha TBD_AT_BANK
  {
    ...
    headSha: '90cc909', predecessorSha: '4e2596d', loc: 181,
    newSiblings: [],
    bankDate: '2026-06-14', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint CL-3c T-CL3c-ConstHardcode-Remainder · CLEANUP ARC sprint 3c · closes const-hardcode mechanism · 7 small clusters (vendor-portal 6 + distributor 4 + payout 3 + servicedesk/settings 3 + masters 2 + fincore 1 + salesx 1 = 20 files) · removed module-scope `const E = 'DEMO'` / `const ENTITY = DEFAULT_ENTITY_SHORTCODE` / `const entityCode = DEFAULT_ENTITY_SHORTCODE` · replaced with `const { entityCode } = useEntityCode()` at component TOP LEVEL · for distributor panels with default-param wrappers (CreditApprovalQueue/DistributorDisputeQueue/DistributorHierarchyMaster) + salesx ProformaInvoicePrintPanel: prop made truly optional + hook fallback inside component, outer default export drops the literal · servicedesk/settings (CommissionRates/EmailTemplates/TellicallerTriggers) const E renamed via `const { entityCode: E } = useEntityCode()` to minimise diff · masters/SchemeMaster: threaded entityCode through readSchemes/writeSchemes/blankScheme + refactored module-scope SCHEME_IMPORT_SCHEMA into buildSchemeImportSchema(entityCode) factory wrapped in useMemo · masters/CustomerSegmentMaster: threaded entityCode through readSegments/writeSegments/seedDemoSegments/blankSegment · fincore/AssetCentreMaster (financial — entity correctness verified): hook swap + removed DEFAULT_ENTITY_SHORTCODE fallback in logAudit (entity is always resolved) · DistributorRateUs: hook aliased `{ entityCode: ENTITY }` to minimise diff · removed every now-unused DEFAULT_ENTITY_SHORTCODE import (ESLint --max-warnings 0 lesson from CL-2b) · 7 NEW GUARD TESTS (one per cluster): src/__tests__/cl-3c/{vendor-portal,distributor,payout,servicedesk-settings,masters,fincore,salesx}-entity-hook-guard.test.ts mirror CL-3a/3b guard, ROOT pointed at each cluster dir, assert offenders.toEqual([]) · TSC 0 · ESLint 0/0 (repo-wide) · Vitest 7/7 PASS guards · ZERO new SIBLINGs · NON-GOAL (deferred): components(16)+mobile(15) = CL-3d, the 23 raw getItem('active_entity_code') readers = CL-3e · predecessor 90cc909 · headSha TBD_AT_BANK
  {
    ...
    headSha: 'b779305', predecessorSha: '90cc909', loc: 220,
    newSiblings: [],
    bankDate: '2026-06-14', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint CL-3d T-CL3d-Components-Mobile-HookSweep · CLEANUP ARC sprint 3d · components 10 + pages/mobile 9 P1 const-hardcode sweep + 12 P2 getActiveEntityCode helper sweep · removed module-scope `const E = 'DEMO'` / `const ENTITY = 'DEMO'` / `const ENTITY = DEFAULT_ENTITY_SHORTCODE` · replaced with `const { entityCode } = useEntityCode()` at component TOP LEVEL · components/mobile captures render as <MobileXCapture/> = hook-legal top-level only · 12 getActiveEntityCode helper files converted: deleted helper, added hook at component top level, replaced uses · guard-hole hotfix: expanded guard regex patterns to catch `const E = 'DEMO'` and `const ENTITY = 'DEMO'` (plus existing DEFAULT_ENTITY_SHORTCODE) — 11 additional files caught and converted · 3 NEW GUARD TESTS (components + pages/mobile + mobile-no-raw-entity) + 1 behavioural test (mobile-capture) · TSC 0 · ESLint 0/0 (repo-wide) · Vitest guards green · ZERO new SIBLINGs · predecessor 462cac9 · headSha b779305
  {
    sprintNumber: 'CL3d' as unknown as number, code: 'T-CL3d-Components-Mobile-HookSweep', composite: false, grade: 'A',
    headSha: 'b779305', predecessorSha: '462cac9', loc: 310,
    newSiblings: [],
    bankDate: '2026-06-14', provenance: 'CONFIRMED',
  },
  // 🆕 Sprint CL-3e T-CL3e-RawKey-Final-EntityResolution · CLEANUP ARC sprint 3e (FINAL entity-resolution sprint) · closes Mechanism 1 (raw-key readers) · 11 non-mobile files reading localStorage.getItem('active_entity_code') (a key with 0 writers → always falls back to DEMO/DEFAULT) + 1 customer/Statement.tsx straggler converted · S1 helper-style (4 files: gateflow/panels.tsx, gateflow/vehicle-panels.tsx, qualicheck/panels.tsx, qualicheck/operational-panels.tsx): module-scope `function getActiveEntityCode()` deleted, each exported component got its OWN `const { entityCode } = useEntityCode()` at top level (vehicle-panels.tsx WeighDialog/VehicleMasterPanel/DriverMasterPanel/WeighbridgeTicketRegisterPanel use `{ entityCode: entity }` alias to minimise diff; VehicleQueuePanel simplified to `const entity = entityCode`) · S2 inline-style (7 files: VendorPortalWelcome + 5 vendor-portal panels Msme43BhTracker/VendorActivityMonitor/VendorBroadcastConsole/VendorCommunicationLogAdmin/VendorScoring + fincore/RCMComplianceReport): stale-capture `useMemo(() => getItem(...) ?? DEFAULT_ENTITY_SHORTCODE, [])` lazy-initializer replaced with reactive `const { entityCode } = useEntityCode()`; RCMComplianceReport default export (GST compliance report — wrong entity = wrong GST data) verified · customer/Statement.tsx:61 `const entityCode = DEFAULT_ENTITY_SHORTCODE` → hook · removed every now-unused DEFAULT_ENTITY_SHORTCODE import · COMPREHENSIVE GUARD src/__tests__/cl-3e/entity-resolution-final-guard.test.ts scans src/pages + src/components recursively, asserts 0 occurrences of ALL 5 variants (getItem('active_entity_code'), function getActiveEntityCode, const E='DEMO', const ENTITY='DEMO', =DEFAULT_ENTITY_SHORTCODE;) — repo-wide entity-resolution completeness proof, no CL-3d-style blind spot · behavioural test src/__tests__/cl-3e/entity-resolution-behavioural.test.ts (gateflow + 6 vendor-portal/customer files) · TSC 0 · ESLint 0/0 (repo-wide) · Vitest 6/6 PASS (1 guard + 5 behavioural) · ZERO new SIBLINGs · NON-GOALS (deferred to founder design call): Class B state-propagation across J1/J2/J4 chains, Class C dormant QA event bus J4 · predecessor b779305 · headSha TBD_AT_BANK
  {
    ...
    headSha: 'f02c23f', predecessorSha: 'b779305', loc: 130,
    newSiblings: [],
    bankDate: '2026-06-14', provenance: 'CONFIRMED',
  },
