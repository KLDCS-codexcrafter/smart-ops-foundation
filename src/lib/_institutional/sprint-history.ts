...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🆕 Sprint RPT-10a T-RPT10a-Executive-Cockpits · Reporting Arc · Phase D cockpits 1 of 2 · 3 executive cockpit pages composing FROZEN primitives + Fin-hub DSC repair · Block 1: 4 NEW DSC sources (receivx.ar · payout.payments · billpassing.bills · eximx.shipments) wrapping existing storage/engines (outstandingKey · vendorPaymentBatchKey · billPassingKey · loadShippingBills) — un-empties the 4 Fin-hub Report Builder previews · dsc-card-id-integrity.test.ts extended (every mounted-builder card resolves ≥1 source · meta cards 'cc' + 'ix' excluded as documented) · Block 2: PromoterCockpitPage at /erp/command-center/promoter (TV mode · 4 auto-cycling sections @ 12s — Org KPIs via deriveRoleDashboard('tenant_admin','management',…) · Cash & AR (xc-cash-position empty + receivx.ar aging) · Compliance (comply360.aggregate.compliance-pct RAG tiles) · Ops pulse (production.orders + dispatch.shipments) · pauses on click/key · integrity badge + honest empty-state per section) · Block 3: CreditXRayPage in ReceivX (rx-credit-xray module · sidebar entry · receivx.ar aging waterfall + top-10 exposure + overdue-% RAG + TableChartToggle drill · distributor.orders gated to management via layerCeilingFor) · Block 4: SpendFunnelPage in Procure360 (spend-funnel module · sidebar entry · funnel by status — only stages with real data render · missing stages declared not faked · budget-vs-actual combo · top-10 vendor concentration) · 5 KPI seeds (xc-promoter · rx-credit-exposure · rx-overdue-pct · pr-spend-funnel · pr-vendor-concentration · idempotent · seed data only) · ZERO new SIBLINGs (pages compose frozen engines · DSC seeds are data) · walls held: ReportBuilder + report-builder-engine + report-definitions FROZEN · all banked pages incl. OEEDashboard + Comply360 audit pages 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'RPT10a' as unknown as number, code: 'T-RPT10a-Executive-Cockpits', composite: false, grade: 'A',
    headSha: 'TBD_AT_BANK', predecessorSha: 'be771e8', loc: 720,
    newSiblings: [],
