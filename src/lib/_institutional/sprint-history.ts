...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🆕 Sprint A.3 T-A3-ServiceDesk-Capstone · Pillar-A · ServiceDesk capstone + 3 NEW outbound bridges (#13 LIVE OEM portal warranty claim · #14 SEAM-ONLY customer health to InsightX · S22 absent · #15 LIVE service trends snapshot to InsightX) · NEW SIBLING servicedesk-capstone-engine (sole engine credit · pure aggregator · reads ServiceTicket + OEMClaim from localStorage walls · NEVER mutates · NO new audit type · NO new RLS surface) · 5 phase2-preview page promotions in src/pages/erp/servicedesk/phase2-preview/ (S36 PSU/Gov Service Contract · S37 Multi-Currency Export Service · S38 IoT-Ready Foundation = 3 Tier-L FULL · S39 Service Performance Benchmark · S40 Engineer Reputation Rating = 2 Tier-L FOUNDATION with explicit Wave-2 honesty banner · routing imports in ServiceDeskPage.tsx 0-DIFF · activeModule cases 0-DIFF · sidebar 0-DIFF · the 5 stub bodies replaced in place per promote-in-place spec) · bridge #14 SEAM-ONLY registered with reason 'S22_absent' · activates the day S22 customer-health engine lands · NO health-score recomputation in A.3 (S22 scope · iron-canon respected) · bridge #13 follows emitOEMClaimPacketToProcure360 pattern with NEW key 'oem_portal_warranty_claim_stub_v1' · CONSUMES existing OEMClaimPacket shape via buildOEMPortalPacket (no duplicate claim logic) · bridge #15 builds tenant-level ServiceTrendsSnapshot via capstone aggregator + emits to insightx outbox · walls held: servicedesk-engine.ts 0-DIFF (110 exports unchanged · consumed read-only) · the 12 existing bridges 0-DIFF (#1-#12 byte-identical · only #13/#14/#15 added · grep enforced) · service-ticket.ts/oem-claim.ts/amc-record.ts types 0-DIFF (read-only) · audit-trail-engine 0-DIFF (no new audit_type) · hash-chain 0-DIFF · retention 0-DIFF · applications.ts/entitlements/routes/ServiceDeskSidebar.types.ts 0-DIFF · phase2-preview file paths preserved (no moves) · NO duplicate page bodies outside phase2-preview · per-page tier table + per-bridge no-duplicity table in A3_close_summary.md · TXUI-6 flipped to 10b7ac12 · 100→101 ⭐ · headSha TBD_AT_BANK
  {
    sprintNumber: 'A3' as unknown as number, code: 'T-A3-ServiceDesk-Capstone', composite: false, grade: 'A',
    headSha: 'TBD_AT_BANK', predecessorSha: '10b7ac12', loc: 1300,
