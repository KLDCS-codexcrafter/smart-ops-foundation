...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🆕 Sprint A.4-Residual T-A4R-Dispatch-Residual · Pillar-A · Bucket-3 (7 Tier-L FT items not absorbed by WMS) · NEW SIBLING dispatch-residual-engine (sole engine credit · CONSUMES existing dispatch/WMS/FinCore surfaces · NEVER rebuilds): bookSampleExpense (005 · wraps sample-expense-voucher-engine.postSampleExpenseVoucherForSOM + postMarketingExpenseVoucherForDOM · no duplicate accounting) · returnRefundableSampleToStock (006 · wraps postStockTransferForReturnedSampleSOM) · triggerPackingReplenishment (007 · reads PackingMaterialMaster levels · suggestions only) · compareCourierRates (022 · ranks TransporterRateCard rate_per_kg × max(weight,min_chargeable) · honest empty when no cards · NO external courier API · NO GPS · NO ML) · computePackingBomVariance (018 · pure math · planned−actual) · emitBomVarianceToSupplier (018 · SEAM-ONLY outbox stub · reason wave2_supplier_portal_absent · Wave-2 real notify) · summarizeReusablePacking (020 · reads ReturnablePackaging · in_stock/with_customer/returned/damaged/lost/overdue + return_rate_pct) · buildDemoSerialRegister (012 · reads DemoOutwardMemo.items[].serial_no · skips empty serial_no · NEVER fabricates) · buildDispatchAnalyticsSnapshot (023 · aggregates delivery_memos + soms + doms + replenishment + reusable · honest_empty flag) · 6 NEW dispatch surfaces (SampleDemoResidualActions Tx · DemoSerialRegister · CourierRateCompare · DispatchAnalytics · PackingReplenishmentSuggestions · ReusablePackingReturn Reports) all mount PageFloorShell (TXUI floor) · 5 NEW sidebar modules added to DispatchOpsSidebar (additive only · existing module IDs 0-DIFF) · DispatchOpsPage switch cases additive · EXCLUDED Bucket-2 Wave-2: 016 GPS / 017 ML / 021 driver-app (grep=0) · WALLS 0-DIFF: DemoOutwardIssue.tsx + SampleOutwardIssue.tsx (issue/memo core logic byte-identical · residual actions live on NEW surface) · PackingMaterialMaster.tsx 0-DIFF · PackingSlipPrint 0-DIFF · DeliveryMemoEntry 0-DIFF · DispatchExceptions 0-DIFF · EWBMonitor 0-DIFF · all WMS engines 0-DIFF (wms-pick-pack · wms-putaway · wms-manifest) · transporter-rate type 0-DIFF · sample-expense-voucher-engine 0-DIFF (consumed) · fincore-engine 0-DIFF · audit-trail-engine 0-DIFF (no new audit_type · reuses dispatch_txn_event) · hash-chain 0-DIFF · retention 0-DIFF · applications.ts/entitlements/routes 0-DIFF · DispatchOpsSidebar existing entries 0-DIFF (only new entries appended) · A.5 flipped to d9556537 · 102→103 ⭐ · headSha TBD_AT_BANK
  {
    sprintNumber: 'A4R' as unknown as number, code: 'T-A4R-Dispatch-Residual', composite: false, grade: 'A',
    headSha: 'TBD_AT_BANK', predecessorSha: 'd9556537', loc: 1200,
