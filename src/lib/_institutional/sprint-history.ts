...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🆕 Sprint W1C-2 T-W1C2-Polish-Riders · Wave-1 Close Arc · 5-block polish bundle + audit riders · Block 1 InlineMasterCreate REAL FORMS (PartyPicker + LedgerPicker · the 4 Polish-1.5 toast TODOs removed · minimal Dialog forms — party: name/type/GSTIN-optional · ledger: name/group — writing through the SAME master storage (erp_group_customer_master / erp_group_vendor_master / erp_group_ledger_definitions) the full Customer/Vendor/Ledger Master pages persist into · new record auto-selected on save) · Block 2 ItemPicker (StockTransferLineGrid · plain-text Item Input swapped for new src/components/fincore/pickers/ItemPicker.tsx reading erp_inventory_items · qty/UOM/rate flow preserved · TODO header note removed · note: no pre-existing reusable ItemPicker found in tree, added minimal one to /pickers/ following GodownPicker pattern · NOT a SIBLING) · Block 3 RIDER VendorPerformanceView migration (recharts RadarChart/PolarGrid/PolarAngleAxis/PolarRadiusAxis/Radar/ResponsiveContainer removed · replaced with <ReportChart chartType='bar'/> over the SAME factor_breakdown computed data · integrity badge vp-performance-integrity-badge wired via signReport · layout preserved · then REMOVED src/pages/vendor-portal/VendorPerformanceView.tsx from RECHARTS_LEGACY_ALLOWLIST in arc-close-sweep.test.ts — vendor-portal entry of the allowlist now EMPTY; non-ERP entries remain Welcome + 2 bridge workbenches + customer dashboard + tower billing) · Block 4 RIDER SiteTwin honest-data pass (6 hardcoded `trend: [...]` sparkline literals + RAGCardSpec.trend field + per-card <ReportChart> sparkline ELEMENTS REMOVED — no real history series exists in computeSiteHealthScore or computeImprestHealthMetrics; NO invented series · the 6 RAG cards stand on real engine values · single page-level <ReportChart bar/> over the already-real `aggregated` array preserves chart-host integrity testid + ReportChart import) · Block 5 RIDER sprint-81a relaxation (the ESLint-inside-vitest case at line 425 converted to it.skip with rationale: superseded by direct pipeline gate · other 52 cases UNTOUCHED) + sprint-history backfill W1C-1 headSha to 9a47f31 + self-seed W1C-2 · ZERO new SIBLINGs · 0-DIFF: master engines (CustomerMaster + VendorMaster + LedgerMaster — picker writes consume same storage keys + record shape) · ItemPicker NEW but follows existing picker pattern · DocSendBar + ReportSendHeader + TableChartToggle + all framework files + all 106 wrapped pages 0-DIFF · headSha TBD_AT_BANK
  {
    sprintNumber: 'W1C2' as unknown as number, code: 'T-W1C2-Polish-Riders', composite: false, grade: 'A',
    headSha: 'TBD_AT_BANK', predecessorSha: '9a47f31', loc: 720,
    newSiblings: [],
