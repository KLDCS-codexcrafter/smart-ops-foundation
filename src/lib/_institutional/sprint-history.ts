...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🆕 Sprint SP.2 T-SP2-Prudent360-ERP · SaaS Productization · EXTENDS SP.1's product-variant-engine ADDITIVELY (NO new SIBLING · empty newSiblings) · ProductVariant gains product_kind ('erp'|'module'|'addon'|'bundle') + enabled_cards: CardId[] + full LimitSet (companies · users · space_gb · branches · transactions_per_month · retention_years · api_calls · support_tier · feature_flags · extra) + PricingPlan (model: per_seat|per_company|flat_tier|usage|hybrid · base_price · per_user/per_company/per_gb · billing_cycle · discount_pct · trial_days · channel_margin_pct) · SP.1 fields preserved 0-DIFF (VariantLimits kept · LimitSet extends it · limits field widened) · resolveErpCardEntitlements DELEGATES to seedDemoEntitlements + filters by enabled_cards (CONSUMES card-entitlement · no in-engine CardEntitlement fabrication · AC3) · validateLimitSet non-negative across all dims (NOT enforced · DP-7) · computeListPrice/computeChannelPrice display math only (NO charging · Wave-2) · Variant Builder gains product_kind selector + 34-card grid (select-all/clear) when kind='erp' + full LimitSet inputs + PricingPlan inputs + read-only list+channel price preview · idempotent seedFlagshipPrudent360 mounts on first load (flagship "Prudent360 ERP" all-cards + "Lite" core-cards + "Manufacturing" mfg-cards) · enabled_cards filtered against real CardId union at write time (no fabricated ids) · honest Wave-2 banner copy widened to mention limits AND billing · NO new audit type · NO new dependencies · walls held: card-entitlement-engine + card-entitlement.ts + ModulesPage/AddOnsPage + entity-hierarchy + applications.ts + hash-chain + retention all 0-DIFF · SP.1 flipped to 83d28166 · 111→112 ⭐ · headSha TBD_AT_BANK
  {
    sprintNumber: 'SP2' as unknown as number, code: 'T-SP2-Prudent360-ERP', composite: false, grade: 'A',
    headSha: 'TBD_AT_BANK', predecessorSha: '83d28166', loc: 750,
    newSiblings: [],
