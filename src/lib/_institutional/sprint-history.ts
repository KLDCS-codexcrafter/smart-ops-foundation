...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🎬 Sprint 137 T-TaskFlow-A641.1 · 🎬 PHASE 8 OPENER · TaskFlow MVP Core (the 13th card · was coming_soon · DP-P7-2 additive · existing CardIds + every existing card metadata 0-DIFF · status field only flip) · 1 NEW SIBLID taskflow-engine (generic cross-department task delegation · CRUD + status lifecycle open→in_progress→blocked→done · assignee from useEmployees · dept from useOrgStructure (DEPARTMENTS_KEY) · customer/vendor from party-master-engine.loadPartiesByType · entity-scoped localStorage via taskflowKey · audit type taskflow_event under 'mca-roc' · ComplianceModule UNTOUCHED · all 4 read surfaces 0-DIFF · FR-44 read-only consume) + applications.ts TaskFlow status flipped coming_soon → active (other card metadata 0-DIFF · status field only · per FR-44 ruling) + NEW taskflow-shell-config + taskflow-sidebar-config (OWN shell mirror comply360 · NO commandCenterShellConfig borrow · DP-D3-1 self-owned-shell precedent) + TaskFlowSidebar.types (TaskFlowModule union) + TaskFlowPage with activeModule state + renderModule switch + /erp/taskflow route + TaskFlowLandingPage (First-Class Standalone Page #64 · sidebar item + renderModule case · computed Due-Soon strip · NOT a sibling) · DESIGN-DECISION-FLAGs: (1) applications.ts at src/components/operix-core/applications.ts IS card registry; (2) Picker SSOTs: useEmployees + useOrgStructure (DEPARTMENTS_KEY) + party-master-engine.loadPartiesByType('customer'|'vendor'); (3) TaskFlow notifications ride sonner toast() + computed Due-Soon strip pending B.4 Notifications Consolidation — Notification type ships as TYPE DEFINITION ONLY · NO store · push-notification-bridge.ts UNTOUCHED; (4) Vitest baseline floor 20-failed/5730-passed/3-skipped (matches S135/S136 posture · pre-existing failures OUT OF SCOPE) · SCOPE WALL: MVP CRUD + status + pickers + Due-Soon strip + 2 sonner event toasts ONLY — NO approvals workflow (Phase 2) · NO notification rail (B.4 future) · NO push-bridge · NO ML · LEAN-BEHAVIORAL TEST POSTURE (≥20 it · S137 own headSha via toContain([...]) NOT toBe · baseline-floor honored · 4 read surfaces 0-DIFF asserted · §N is a FLOOR · S137 last entry) · 60-streak ⭐ target · headSha TBD_AT_BANK (final open entry · legitimately last)
  {
