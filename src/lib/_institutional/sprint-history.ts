...
export const SPRINTS: SprintEntry[] = [
  ...
  // 🎬 Sprint WMS3 T-WMS3-Manifest-Ship · WMS-ARC 3 of 3 · ARC CLOSE · Manifests + Shipments + Package-Types + Tolerance-Groups + EximX export routing · NEW SIBLING wms-manifest-engine.ts (sole engine credit · package-type + tolerance-group masters CRUD · createShipmentsFromPacked (W1 packed-group source) + createExportShipment (EximX export PO + dispatch-mirror READ-ONLY · canon-5 proof = zero EximX writes) · buildManifest/finalizeManifest/recordHandover lifecycle (status flow draft→finalized→acknowledged|discrepancy · shipments packed→manifested→handed_over) · acknowledgeManifest = LR-acceptance pattern verbatim (Dispatch writes manifests · Logistics writes ONLY ack ledger) · checkToleranceAndDispute (founder DP: breach requires BOTH pct AND abs exceeded → createDisputeFromMatch via existing dispute-workflow-engine CONSUMED never modified · within tolerance → recorded as accepted_variance never silent write-off · AC7) · getManifestForExportPO = the flip read API for EximX rider + Wave-2 · getManifestSummary chips) · 3 new record types born under P8.6 floor (Manifest → gst_8yr e-way/GST-adjacent transport doc · Shipment + ManifestAck → operational_log_only · additive RECORD_TYPE_POLICY_MAP cases — the only permitted retention-engine touch) · audit literal dispatch_txn_event REUSED (no new literal) · 1 Dispatch console page (ManifestConsole · honesty line verbatim "Courier label generation, live tracking and e-way bill integration arrive with Wave-2.") · 1 Logistics page (LogisticManifestQueue · LR-queue pattern · writes ONLY acknowledgeManifest API · zero direct localStorage.setItem on Dispatch stores · AC6 greppable) · 30-LOC EximX rider on ExportDispatchList.tsx (read-only badge "Leg-1 · Dispatch manifest {no} · {status}" · no store write · no other EximX file touched) · App.tsx additive route /erp/logistic/manifest-queue · LogisticDashboard additive nav card · DispatchHubSidebar/DispatchHubPage additive only · walls held: dispute-workflow-engine.ts 0-DIFF (consumed) · export-dispatch-bridge.ts + export-dispatch-mirror.ts + egm.ts 0-DIFF · ALL other EximX 0-DIFF · audit-trail-hash-chain + audit-trail-chain-engine + logAudit entry-write 0-DIFF · comply360-audit-retention-engine 0-DIFF · RetentionConsolePage 0-DIFF · packing engines + W1/W2 engines 0-DIFF · applications/entitlements 0-DIFF · NO courier APIs · NO live tracking claims · NO e-way bill integration · WMS-ARC CLOSED · W1→W3 · 86→89 ⭐ · Single-Door canon fully realized: all sources picked (W1) · imports through Dispatch inward (W2) · exports through Dispatch manifests (W3) · Uniware floor-parity hole closed · headSha TBD_AT_BANK
  {
    sprintNumber: 'WMS3' as unknown as number, code: 'T-WMS3-Manifest-Ship', composite: false, grade: 'A',
    headSha: 'TBD_AT_BANK', predecessorSha: 'bdd4c6ec', loc: 1500,
