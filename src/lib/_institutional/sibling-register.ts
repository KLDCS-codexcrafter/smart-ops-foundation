...
export const SIBLINGS: SiblingEntry[] = [
  ...
  { id: 'workforce-planning-engine', name: 'Workforce Planning Engine (Sprint 117 · Arc D.0 · Pillar D.0 · headcount projection by org node · hiring/attrition modeling · permanent-vs-contract mix · projected workforce cost flagged against the S116 AOP cost_target (intra-arc linkage) · projectWorkforce/upsertHeadcountPlan/listHeadcountPlans/getWorkforceCostVsAOP/readAopCostTarget/isValidScope/getCapacityContextRowCount · planned_headcount = current + hires − attrition · attrition = round(current × attrition_pct / 100) · projected_cost = headcount × cost_per_head via decimal-helpers · cost_variance_vs_aop = projected_cost − aop_cost_target (skipped honestly when no AOP target set · DP-A4-8) · validates scope_id against the real org tree (rejects orphans) · idempotent upsert by composite key {fy,scope_level,scope_id} · audit type workforce_plan_event under mca-roc · ComplianceModule UNTOUCHED · FR-44 REUSES org-structure (Division/Department) + capacity-planning-engine (computeBottleneckHeatmap probe · 0-DIFF) + org-planning-engine.listStrategicTargets (cost_target · 0-DIFF) + contract-manpower (ContractWorker) + employee (Employee) types — reimplements none, all stay 0-DIFF · SCOPE WALL DP-D0-7: workforce projection only — NO OKR (S118) · NO org-design (S119) · NO budget/forecast/scenario (D.1) · NO performance-management / compensation-planning (deferred to a later HR arc) · scope-wall test asserts those exports DO NOT exist on the engine surface · READS_FROM org-structure + capacity-planning-engine + org-planning-engine + contract-manpower + employee + decimal-helpers + audit-trail-engine)', path: 'src/lib/workforce-planning-engine.ts', sprintAdded: 117, compositeAdded: 117, functionCount: 7, moatsRealized: [], provenance: 'CONFIRMED' },
];
