...
export const SIBLINGS: SiblingEntry[] = [
  ...
  // 🎉 Sprint 112 T-Phase-6.C.2.4 · Arc 3 CAPSTONE · "Horizon 1.5" DELIVERED · Pillar C.2 · Consolidation Disclosure Pack (Schedule III + Ind AS 110) · 1 NEW SIBLING consolidation-disclosure-engine (FR-44 PURE ASSEMBLY · NO figure recompute · NO XBRL taxonomy rebuild) + Page #39 ConsolidatedFinancialsPage EXTENDED (Disclosure tab + Export PDF/XBRL buttons · NOT a new page · NOT a sibling) + 1 new audit type consolidation_disclosure_event under 'mca-roc' (ComplianceModule UNTOUCHED) · 38-streak ⭐ target · headSha TBD_AT_BANK (backfilled at S113 Block 1)
  { id: 'consolidation-disclosure-engine', name: 'Consolidation Disclosure Engine (Sprint 112 · Arc 3 CAPSTONE · "Horizon 1.5" · Schedule III + Ind AS 110 consolidated-financials disclosure pack · FR-44 PURE ASSEMBLY: buildDisclosurePack calls S109 buildConsolidatedPnL + S111 buildBalanceSheet/buildCashFlow/computeNCI/computeGoodwill (CALLED · not recomputed) and maps each section to getSchedIIITaxonomyElements category (balance_sheet/profit_loss/cash_flow/notes) · exportDisclosureXBRL REUSES comply360-xbrl-builder-engine buildXBRL + validateXBRL + exportXBRLDownload via bridge (synthetic aoc4_xbrl_id `consolidation-xbrl-{fy}` + getActiveBAPAccount() — xbrl-builder 0-DIFF · per-element value injection is Arc 4/Phase 8 work · §L-noted) · exportDisclosurePDF REUSES board-pack-pdf-engine jsPDF+autoTable pattern · cross-references form-3ceb-engine loadForm3CEBSnapshots (scope: count of CA-signed snapshots for FY · §L-noted) · schedule_iii_compliant + ind_as_110_compliant flags · ALL money math via decimal-helpers (dSum/round2) · audit type: consolidation_disclosure_event under mca-roc · READS_FROM group-consolidation-engine + consolidated-balance-sheet-engine + consolidated-cash-flow-engine + comply360-xbrl-builder-engine + comply360-audit-framework-engine + form-3ceb-engine · FR-44 WALL: builds NO financial figures · builds NO XBRL taxonomy · SCOPE WALL DP-A3-9: disclosure assembly + PDF/XBRL export ONLY · NO new financial computation · NO OOB · NO Pillar-C.3 (Arc 4) · all sources 0-DIFF)', path: 'src/lib/consolidation-disclosure-engine.ts', sprintAdded: 112, compositeAdded: 112, functionCount: 5, moatsRealized: [], provenance: 'CONFIRMED' },
];
