# CARD DEEP-DIVE · FIN CORE
## Operix Developer Handbook — Card Series

**Verified against:** registry `applications.ts` + card surfaces at freeze `c30f161`.
**Hub:** Fin Hub · **Route:** `/erp/fincore` · **Icon:** Calculator · **Registry id:** `fincore`

---

## 1. What it is / what it's for
Accounting + inventory transactions · sales, purchase, payments, journals, voucher register. The financial heart of the ERP. India compliance baked in (GST/TDS/E-invoice).

## 2. Where it lives
- **Route:** `/erp/fincore`
- **Registry id:** `fincore` (in `src/components/operix-core/applications.ts`)
...
---
*Card deep-dive · Fin Core · verified at c30f161 · Operix Developer Handbook card series.*
