# CARD DEEP-DIVE · PROJX
## Operix Developer Handbook — Card Series

**Verified against:** registry `applications.ts` + card surfaces at freeze `c30f161`.
**Hub:** Sales Hub · **Route:** `/erp/projx` · **Icon:** ClipboardList · **Registry id:** `projx`

---

## 1. What it is / what it's for
Project Management Hub (orchestrator) · multi-line long-running orders with milestones, resource allocation, time entries, project P&L, project-level invoicing. Every other card tags transactions with project_centre_id (D-218).

## 2. Where it lives
- **Route:** `/erp/projx`
- **Registry id:** `projx` (in `src/components/operix-core/applications.ts`)
...
---
*Card deep-dive · ProjX · verified at c30f161 · Operix Developer Handbook card series.*
