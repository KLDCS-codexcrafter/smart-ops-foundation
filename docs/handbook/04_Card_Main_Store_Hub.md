# CARD DEEP-DIVE · MAIN STORE HUB
## Operix Developer Handbook — Card Series

**Verified against:** registry `applications.ts` + card surfaces at freeze `c30f161`.
**Hub:** Ops Hub · **Route:** `/erp/main-store-hub` · **Icon:** Package · **Registry id:** `inventory-hub`

---

## 1. What it is / what it's for
Plant-level Main Store backbone · distributes to Department Stores. Heavy inventory module — GRN, MIN, RTV, Cycle Count, Item Master, Storage Matrix, Batch/Serial tracking, ABC classification, Hazmat profiles, Reorder management.

## 2. Where it lives
- **Route:** `/erp/main-store-hub`
- **Registry id:** `inventory-hub` (in `src/components/operix-core/applications.ts`)
...
- `Main Store Hub inventory surfaces (GRN/MIN/cycle-count)`
...
---
*Card deep-dive · Main Store Hub · verified at c30f161 · Operix Developer Handbook card series.*
