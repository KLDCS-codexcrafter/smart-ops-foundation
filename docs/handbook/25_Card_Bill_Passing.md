# CARD DEEP-DIVE · BILL PASSING
## Operix Developer Handbook — Card Series

**Verified against:** registry `applications.ts` + card surfaces at freeze `c30f161`.
**Hub:** Fin Hub · **Route:** `/erp/bill-passing` · **Icon:** FileText · **Registry id:** `bill-passing`

---

## 1. What it is / what it's for
3-way match invoice processing (PO + GRN + Vendor Invoice). For Procure360 + RequestX + Job Work Out. Feeds PayOut for payment scheduling.

## 2. Where it lives
- **Route:** `/erp/bill-passing`
- **Registry id:** `bill-passing` (in `src/components/operix-core/applications.ts`)
...
- `BillPassingPage`
- `BillPassingRegisterPanel`
- `BillPassingDetailPanel`
...
---
*Card deep-dive · Bill Passing · verified at c30f161 · Operix Developer Handbook card series.*
